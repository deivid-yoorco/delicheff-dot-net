...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual ExcelWorksheet SetBranchesXls(ExcelWorksheet worksheet, List<int> orderIds)
        {
            var row = 1;
            int numCol = worksheet.Dimension.Columns + 1;
            if (numCol > 2)
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (branchResult.IsSuccessStatusCode)
                    {
                        ...
                        worksheet.Cells[row, numCol].Value = "Branch";
                        worksheet.Cells[row, numCol].Style.Fill.PatternType = ExcelFillStyle.Solid;
                        worksheet.Cells[row, numCol].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(184, 204, 228));
                        worksheet.Cells[row, numCol].Style.Font.Bold = true;
                        foreach (var name in results)
                        {
                            ...
                            do
                            {
                                var cell = worksheet.Cells[row, 1].Value;
                                orderId = cell == null ? 0 : (int)cell;
                                row++;
                            } while (orderId < 1);
                            row--;
                            worksheet.Cells[row, numCol].Value = name;
                        }
                    }
                }
            }
        ...
        private byte[] ExportOrderToXlsxWithProducts(PropertyByName<Order>[] properties, IEnumerable<Order> itemsToExport)
        {
            var orderItemProperties = new[]
            {
                ...
                new PropertyByName<OrderItem>("ProductManufacturers", oi => oi.Customizable)
            };
                ...
                using (var xlPackage = new ExcelPackage(stream))
                {
                    ...
                    foreach (var order in itemsToExport)
                    {
                        ...
                        foreach (var orederItem in orederItems)
                        {
                            ...
                            foreach (var manufacturer in orederItem.Product.ProductManufacturers)
                            {
                                if (manufacturer != null)
                                {
                                    manufacturerList.Add(manufacturer.Manufacturer.Name);
                                }
                            }

                            var listManufacturer = string.Join(",", manufacturerList);
                            orderItemsManager.WriteToXlsxWithManufacturers(worksheet, row, _catalogSettings.ExportImportUseDropdownlistsForAssociatedEntities, 2, fpWorksheet, listManufacturer);
                            worksheet.Row(row).OutlineLevel = 1;
                            worksheet.Row(row).Collapsed = true;
                        }
                    ...
                    {
                        var orderIds = itemsToExport.Select(x => x.Id).ToList();
                        worksheet = SetBranchesXls(worksheet, orderIds);
                    }
        ...
        private string GetNewsLetterCustomer(Customer customer)
        {
            ...
            {
                return newsletter.Active.ToString();
            }
        }
        ...
        /// <summary>
        /// Export massive discounts to XLSX
        /// </summary>
        /// <param name="discounts">Discounts</param>
        public virtual byte[] ExportMassiveDiscountsToXlsx(IEnumerable<Discount> discounts)
        {
            //property array
            var properties = new[]
            {
        ...
        public virtual string ExportProductsToXml(IList<Product> products)
        {
            ...
            foreach (var product in products)
            {
                ...
                xmlWriter.WriteString("ParentSKU", product.ParentSku);
        ...
        public virtual byte[] ExportProductsToXlsx(IEnumerable<Product> products)
        {
            var properties = new[]
            {
                ...
                new PropertyByName<Product>("ParentSKU", p => p.ParentSku),
        ...
        /// <summary>
        /// Export products for facebook to XLSX
        /// </summary>
        /// <param name="products">Products</param>
        public virtual byte[] ExportFacebookProductsToXlsx(IEnumerable<Product> products, string storeName, string storeUrl)
        {
            using (ExcelPackage excel = new ExcelPackage())
            {
                excel.Workbook.Worksheets.Add("fb_upload_standard");

                var headerRow = new List<string[]>()
                  {
                    new string[] { "id", "title", "description", "availability", "condition", "price", "link", "image_link", "brand", "manufacturers", "additional_image_link", "color", "gender", "product_type" }
                  };

                string headerRange = "A1:" + Char.ConvertFromUtf32(headerRow[0].Length + 64) + "1";

                var worksheet = excel.Workbook.Worksheets["fb_upload_standard"];
                ...
                {
                    var column = 1;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = product.Sku; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = product.Name; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = product.ShortDescription; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = product.Published ? "in stock" : "out stock"; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = "new"; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = Math.Round(product.Price, 2) + " MXN"; column++;
                    if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea)
                    {
                        var encodedStrings = new List<string>();
                        foreach (var encodedString in product.Name.Split(' '))
                        {
                            encodedStrings.Add(Uri.EscapeDataString(encodedString));
                        }
                        worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = $"{storeUrl}/search?q={string.Join("+", encodedStrings)}"; column++;
                    }
                    else
                    {
                        worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = $"{storeUrl}/{product.GetSeName()}"; column++;
                    }
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = storeUrl + "/Api/Product/ProductImage?id=" + product.Id; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = storeName; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = string.Join(", ", product.ProductManufacturers.Select(x => x.Manufacturer.Name)); column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = storeUrl + "/Api/Product/ProductImage?id=" + product.Id; column++;
                    var specAttr = product.ProductSpecificationAttributes.Select(x => x.SpecificationAttributeOption).ToList();
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = specAttr.Where(x => x.ColorSquaresRgb != null).Count() > 0 ?
                        specAttr.Where(x => x.ColorSquaresRgb != null).FirstOrDefault().Name : ""; column++;
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = specAttr.Where(x => x.Name == "Hombre" || x.Name == "Mujer" || x.Name == "Unisex").Count() > 0 ?
                        specAttr.Where(x => x.Name == "Hombre" || x.Name == "Mujer" || x.Name == "Unisex").FirstOrDefault().Name : "Unisex"; column++;

                    var childrensCategories = string.Join(",", product.ProductCategories.Where(x => x.Category.ParentCategoryId != 0).Select(x => x.Category.Name).Distinct().ToList());
                    var fathersCategories = string.Join(",", product.ProductCategories.Where(x => x.Category.ParentCategoryId == 0).Select(x => x.Category.Name).Distinct().ToList());
                    worksheet.Cells[Char.ConvertFromUtf32(column + 64) + row].Value = string.IsNullOrEmpty(childrensCategories) ? fathersCategories : childrensCategories; column++;
        ...
        public virtual byte[] ExportOrdersToXlsx(IList<Order> orders)
        {
            ...
            var properties = new[]
            {
                ...
                new PropertyByName<Order>("CreatedOn", p => p.CreatedOnUtc.ToLocalTime().ToString("dd/MM/yyyy")),
                new PropertyByName<Order>("VisitingDay", p => ((p.SelectedShippingDate ?? DateTime.MinValue).ToString("dd-MM-yyyy") + " - " + p.SelectedShippingTime.Split(new string[] { " [" }, StringSplitOptions.None)[0])?? string.Empty),
        ...
        public virtual byte[] ExportShoppingListToXlsx(IList<Order> orders, DateTime selectedDate)
        {
            ExcelPackage excelPackage = new ExcelPackage();
            ExcelWorksheet workSheet = excelPackage.Workbook.Worksheets.Add(selectedDate.ToString("dd-MM-yyyy")); // Sheets's name
            ...
            workSheet.Cells["A1"].Value = "CENTRAL EN LÍNEA";
            workSheet.Cells["A1"].Style.Font.Bold = true;
            workSheet.Cells["A1:E1"].Merge = true;

            workSheet.Cells["A2"].Value = "Lista de compras para el " + selectedDate.ToString("dd-MM-yyyy");
            ...
            workSheet.Cells["B4"].Value = "Piezas";
            workSheet.Cells["B4"].Style.Font.Bold = true;

            workSheet.Cells["C4"].Value = "Peso";
            workSheet.Cells["C4"].Style.Font.Bold = true;

            workSheet.Cells["D4"].Value = "Condición";
            workSheet.Cells["D4"].Style.Font.Bold = true;

            workSheet.Cells["E4"].Value = "Precio";
            workSheet.Cells["E4"].Style.Font.Bold = true;

            var items = orders.Select(x => x.OrderItems);
            ...
            var groups = orderItems.OrderBy(x => x.Product.Name).GroupBy(x => new { x.Product.Id, x.SelectedPropertyOption }).ToList();
            for (int i = 0; i < groups.Count(); i++)
            {
                var weight = "-";
                var extra = string.Empty;
                if (groups[i].FirstOrDefault().EquivalenceCoefficient > 0)
                {
                    weight = (groups[i].Sum(x => x.Quantity) / groups[i].FirstOrDefault().EquivalenceCoefficient).ToString("0.##") + " kg";
                }
                else if (groups[i].FirstOrDefault().WeightInterval > 0)
                {
                    weight = ((groups[i].Sum(x => x.Quantity) * groups[i].FirstOrDefault().WeightInterval) / 1000).ToString("0.##") + " kg";
                }

                workSheet.Cells["A" + (i + 5)].Value = groups[i].Select(x => x.Product.Name).FirstOrDefault();
                workSheet.Cells["B" + (i + 5)].Value = groups[i].FirstOrDefault().WeightInterval > 0 ? "-" : groups[i].Sum(x => x.Quantity).ToString();
                workSheet.Cells["C" + (i + 5)].Value = weight;
                workSheet.Cells["D" + (i + 5)].Value = groups[i].Select(x => x.SelectedPropertyOption).FirstOrDefault() ?? "-";
                workSheet.Cells["E" + (i + 5)].Value = "$" + groups[i].Max(x => x.UnitPriceInclTax).ToString("0.##");
            }
        ...
        public virtual byte[] ExportCustomersToXlsx(IList<Customer> customers)
        {
            ...
            var properties = new[]
            {
                ...
                new PropertyByName<Customer>("Phone", p => GetCustomerPhoneNumber(p)),
                ...
                new PropertyByName<Customer>("ActiveNewsLetter",  GetNewsLetterCustomer),
                new PropertyByName<Customer>("DateOfBirth", p => p.GetAttribute<string>(SystemCustomerAttributeNames.DateOfBirth)),
        ...
        protected virtual string GetCustomerPhoneNumber(Customer customer)
        {
            var phoneNumber = customer.GetAttribute<string>(SystemCustomerAttributeNames.Phone);
            if (string.IsNullOrEmpty(phoneNumber))
            {
                var address = customer.Addresses.OrderByDescending(x => x.CreatedOnUtc).FirstOrDefault();
                if (address != null)
                {
        ...
        public virtual string ExportCustomersToXml(IList<Customer> customers)
        {
            ...
            foreach (var customer in customers)
            {
                ...
                xmlWriter.WriteElementString("Phone", null, GetCustomerPhoneNumber(customer));

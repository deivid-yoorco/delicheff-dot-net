    ...
    public partial class ImportManager : IImportManager
    {
        ...
        public virtual void ImportProductsFromXlsx(Stream stream)
        {
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                var allProductsBySku = _productService.GetProductsBySku(allSku.ToArray(), _workContext.CurrentVendor?.Id ?? 0, TeedCommerceStores.CurrentStore == TeedStores.Hamleys ? true : false);
                ...
                for (var iRow = 2; iRow < endRow; iRow++)
                {
                    ...
                    var product = skuCellNum > 0 ? allProductsBySku.FirstOrDefault(p => p.Sku.TrimStart('0') == manager.GetProperty("SKU").StringValue.TrimStart('0')) : null;
                    ...
                    foreach (var property in manager.GetProperties)
                    {
                        switch (property.PropertyName)
                        {
                            ...
                            case "SKU":
                                ...
                                break;
                            case "ParentSKU":
                                product.ParentSku = property.StringValue;
                                break;
                            ...
                            case "StockQuantity":
                                ...
                                if (property.IntValue != previousStockQuantity && product.Id > 0)
                                {
                                    _stockLogService.InsertStockLog(new StockLog()
                                    {
                                        CreatedOnUtc = DateTime.UtcNow,
                                        NewStock = product.StockQuantity,
                                        OldStock = previousStockQuantity,
                                        Product = product.Name,
                                        ProductId = product.Id,
                                        SKU = product.Sku,
                                        User = _workContext.CurrentCustomer.GetFullName(),
                                        ChangeType = TypeChangeEnum.Excel
                                    });
                                }
                                break;
                            ...
                            case "Price":
                                ...
                                if(property.DecimalValue != previousPrice && product.Id > 0)
                                {
                    ...
                    if (isNew && properties.All(p => p.PropertyName != "AllowAutoUpdatePrice"))
                        product.AllowAutoUpdatePrice = true;

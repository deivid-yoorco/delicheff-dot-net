...
namespace Nop.Services.ExportImport.Help
{
    ...
    public class PropertyManager<T>
    {
        ...
        public void WriteToXlsxWithManufacturers(ExcelWorksheet worksheet, int row, bool exportImportUseDropdownlistsForAssociatedEntities, int cellOffset = 0, ExcelWorksheet fWorksheet = null, string listManufacturer = "")
        {
            if (CurrentObject == null)
                return;
            ...
            foreach (var prop in _properties.Values)
            {
                ...
                {
                    var valueProp = prop.GetProperty(CurrentObject);
                    if (valueProp.Equals(false) || valueProp.Equals(true))
                    {
                        cell.Value = listManufacturer;
                    }
                    else
                    {

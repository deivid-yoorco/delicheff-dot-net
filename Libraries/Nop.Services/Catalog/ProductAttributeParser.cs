...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductAttributeParser : IProductAttributeParser
    {
        ...
        {
            // If both XMLs of the same product are empty then return true
            if (string.IsNullOrEmpty(attributesXml1) && string.IsNullOrEmpty(attributesXml2))
                return true;
        ...
        /// <param name="ignoreNonCombinableAttributes">A value indicating whether we should ignore non-combinable attributes</param>
        /// <returns>Found product attribute combination</returns>
        public virtual List<ProductAttributeCombination> FindAllProductAttributeCombinations(Product product,
            string attributesXml, bool ignoreNonCombinableAttributes = true)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            ...
            return combinations.Where(x =>
                AreProductAttributesEqual(x.AttributesXml, attributesXml, ignoreNonCombinableAttributes)).ToList();
        }

        /// <summary>
        /// Get the products combinations by generating an xml of the product attribute value
        /// </summary>
        /// <param name="product">Product</param>
        /// <param name="currentAttribute">Product Attribute Value</param>
        /// <param name="withStock">Specify if stock should be bigger than 0</param>
        /// <returns>View path</returns>
        public virtual List<ProductAttributeCombination> GetCombinationsByProductAttributeValue(Product product, ProductAttributeValue currentAttribute, bool withStock = true)
        {
            var total = new List<ProductAttributeCombination>();
            var currentAttributeXml =
                string.Format(@"<Attributes>
                    ...
                    </Attributes>", currentAttribute?.ProductAttributeMappingId ?? 0, currentAttribute?.Id ?? 0);
            var combinations = FindAllProductAttributeCombinations(product, currentAttributeXml).ToList();
            if (withStock)
                combinations = combinations.Where(x => x.StockQuantity > 0).ToList();

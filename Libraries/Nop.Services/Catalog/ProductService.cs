...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <param name="includeRewards">
        /// null - process "Published" property according to "showHidden" parameter
        /// true - load only "Published" products
        /// false - load only "Unpublished" products
        /// </param>
        /// <param name="includedProductIds">Adds product identifiers to the search</param>
        /// <param name="calculateLevenshteinDistance">A value indicating whether to order result by levenshtein distance</param>
        /// <returns>Products</returns>
        protected virtual IPagedList<Product> SearchProductsUseLinq(ref IList<int> filterableSpecificationAttributeOptionIds, ref bool notFoundOptimized,
            ...
            IList<int> filteredSpecs, ProductSortingEnum orderBy, bool showHidden, bool? overridePublished, bool includeRewards, bool onlyRewards,
            List<int> includedProductIds = null, bool calculateLevenshteinDistance = false)
        {
            notFoundOptimized = false;
            //products
            var query = _productRepository.Table;
            query = query.Where(p => !p.Deleted);
            if (!includeRewards)
            {
                query = query.Where(x => x.RewardPointsRequired == 0);
            }
            if (onlyRewards)
            {
                query = query.Where(x => x.RewardPointsRequired > 0);
            ...
            {
                int resultsCount = 0;
                var searchArray = keywords.Replace("  ", " ").Split(' ').Where(x => x.Count() > 2).ToList();
                if (keywords.ToLower() == "te" || keywords.ToLower() == "té")
                {
                    searchArray = new List<string>() { "te " };
                }

                if (searchArray.Count == 0)
                    searchArray = new List<string>() { keywords };

                for (int i = 0; i < searchArray.Count(); i++)
                    searchArray[i] = Singularize(searchArray[i]);

                while (resultsCount == 0 && searchArray.Count > 0)
                {
                    keywords = string.Join(" ", searchArray);

                    query = query.Where(x => searchArray.All(y => x.Name.Contains(y)) ||
                                            (searchDescriptions && x.FullDescription.Contains(keywords)) ||
                                            (searchDescriptions && x.ShortDescription.Contains(keywords)) ||
                                            (searchSku && x.Sku == keywords) ||
                                             x.Gtin == keywords ||
                                            (searchProductTags && x.ProductTags.Select(y => y.Name).Contains(keywords)) ||
                                             x.ProductManufacturers.Select(y => y.Manufacturer.Name).Contains(keywords))
                                 .OrderBy(x => x.Name.IndexOf(keywords)).ThenBy(x => x.Name);

                    resultsCount = query.Count();
                    if (resultsCount == 0 && searchArray.Count > 0)
                    {
                        notFoundOptimized = true;
                        searchArray.RemoveAt(searchArray.Count - 1);
                    }
                }
            ...
            else if (orderBy == ProductSortingEnum.Position && string.IsNullOrWhiteSpace(keywords))
            {
            ...
            //add products with given ids if any
            if (includedProductIds?.Any() ?? false)
            {
                query = query.Where(x => includedProductIds.Contains(x.Id));
            }

            if (!string.IsNullOrWhiteSpace(keywords) && calculateLevenshteinDistance &&
                keywords.Trim() != "te" && keywords.Trim() != "té")
            {
                var productLists = query.ToList().OrderBy(x => StringHelpers.CalculateLevenshteinDistance(keywords, x.Name))
                        .ToList();

                var finalProducts = new PagedList<Product>(productLists, pageIndex, pageSize);
        ...
        private string Singularize(string text)
        {
            ...
            if (string.Concat(text[text.Length - 2], text[text.Length - 1]) == "es")
            {
        ...
        /// <returns></returns>
        public virtual IQueryable<Product> GetAllProductsQuery(bool includeDeleted = false)
        {
            var query = _productRepository.Table;
            if (!includeDeleted)
        ...
        public virtual IList<Product> GetUnpublishedProducts()
        {
            return _productRepository.Table.Where(x => !x.Deleted && !x.Published).ToList();
        }

        public virtual IList<Product> GetPublishedProducts()
        {
            return _productRepository.Table.Where(x => !x.Deleted && x.Published).ToList();
        }
        ...
        /// false - load only "Unpublished" products
        /// </param>
        /// <param name="includeRewards">A value indicating if should return reward products</param>
        /// <param name="onlyRewards">A value indicating if should return only reward products</param>
        /// <param name="includedProductIds">Adds product identifiers to the search</param>
        /// <param name="calculateLevenshteinDistance">A value indicating whether to order result by levenshtein distance</param>
        /// <returns>Products</returns>
        public virtual IPagedList<Product> SearchProducts(
            ...
            List<int> includedProductIds = null,
            bool calculateLevenshteinDistance = false)
        {
            return SearchProducts(out IList<int> _, true,
                ...
                orderBy, showHidden, overridePublished, includeRewards, onlyRewards,
                includedProductIds, calculateLevenshteinDistance);
        }
            ...
            bool calculateLevenshteinDistance = false)
        {
            ...
            return SearchProductsUseLinq(ref filterableSpecificationAttributeOptionIds, ref _, loadFilterableSpecificationAttributeOptionIds, pageIndex, pageSize, categoryIds, manufacturerId, storeId, vendorId, warehouseId, productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts, priceMin, priceMax, productTagId, keywords, searchDescriptions, searchManufacturerPartNumber, searchSku, searchProductTags, searchLocalizedValue, allowedCustomerRolesIds, languageId, filteredSpecs, orderBy, showHidden, overridePublished, includeRewards, onlyRewards, includedProductIds, calculateLevenshteinDistance);
        }

        /// <summary>
        /// Search products
        /// </summary>
        /// <param name="notFoundOptimized">Check if not found products and use an optimized result</param>
        /// <param name="pageIndex">Page index</param>
        /// <param name="pageSize">Page size</param>
        /// <param name="categoryIds">Category identifiers</param>
        /// <param name="manufacturerId">Manufacturer identifier; 0 to load all records</param>
            ...
            bool calculateLevenshteinDistance = false)
        {
            ...
            return SearchProductsUseLinq(ref _, ref notFoundOptimized, true, pageIndex, pageSize, categoryIds, manufacturerId, storeId, vendorId, warehouseId, productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts, priceMin, priceMax, productTagId, keywords, searchDescriptions, searchManufacturerPartNumber, searchSku, searchProductTags, searchLocalizedValue, allowedCustomerRolesIds, languageId, filteredSpecs, orderBy, showHidden, overridePublished, includeRewards, onlyRewards, calculateLevenshteinDistance: calculateLevenshteinDistance);
        }
        ...
        public virtual List<Product> GetGiftProducts()
        {
            return _productRepository.Table.Where(x => x.GiftProductEnable && !x.Deleted && x.Published).ToList();
        }
        ...
        /// <returns>Products</returns>
        public IList<Product> GetProductsBySku(string[] skuArray, int vendorId = 0, bool checkWithoutLeadingZeros = false)
        {
            if (skuArray == null)
            ...
            if (checkWithoutLeadingZeros)
            {
                var parsed = skuArray.Select(x => x.TrimStart('0'));
                var products = query.AsEnumerable().Where(p => !p.Deleted && (skuArray.Contains(p.Sku.TrimStart('0')) || parsed.Contains(p.Sku.TrimStart('0'))));
        ...
        public virtual void AdjustInventory(Product product, int quantityToChange, string attributesXml = "", string message = "")
        {
            ...
            if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
            {
                ...
                if (combination != null)
                {
                    ...
                    if (!product.Published && !product.VisibleIndividually && !product.ShowOnHomePage && combination.StockQuantity > 0)
                    {
                        product.Published = true;
                        product.VisibleIndividually = true;
                        product.ShowOnHomePage = true;
                    ...
                    {
                        var qty = 0;
                        var allXmlAttributes = _productAttributeParser.GenerateAllCombinations(product);
                        foreach (var xmlAttributes in allXmlAttributes)
                        {
                            var attrProduct = _productAttributeParser.FindProductAttributeCombination(product, xmlAttributes);
                            if (attrProduct == null) continue;
                            qty += attrProduct.StockQuantity;
                        }

...
namespace Nop.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
        ...
        public virtual decimal CalculateGroceryPrice(Product product, ShoppingCartItem shoppingCartItem, int quantity, bool isRemoving, OrderItem orderItem = null)
        {
            if (orderItem != null)
            {
                if (orderItem.PriceInclTax == orderItem.UnitPriceInclTax) return orderItem.PriceInclTax;
                if (orderItem.Product.EquivalenceCoefficient > 0 || orderItem.Product.WeightInterval > 0)
                {
                    var qty = orderItem.Quantity;
                    if (orderItem.Product.EquivalenceCoefficient > 0)
                    {
                        return (qty * orderItem.UnitPriceInclTax) / orderItem.Product.EquivalenceCoefficient;
                    }
                    else
                    {
                        return ((qty * orderItem.Product.WeightInterval) * orderItem.UnitPriceInclTax) / 1000;
                    }
                }
                else
                {
                    var qty = quantity != orderItem.Quantity ? quantity : orderItem.Quantity;
                    return (qty * orderItem.UnitPriceInclTax);
                }
            }
            else
            {
                ...
                {
                    var qty = 0;
                    if (shoppingCartItem == null) qty = quantity;
                    else qty = isRemoving ? shoppingCartItem.Quantity - quantity : shoppingCartItem.Quantity + quantity;
                    if (product.EquivalenceCoefficient > 0)
                    {
                        return (qty * product.Price) / product.EquivalenceCoefficient;
                    }
                    else
                    {
                        return ((qty * product.WeightInterval) * product.Price) / 1000;
                    }
                }
                else
                {
                    var qty = shoppingCartItem == null ? quantity : shoppingCartItem.Quantity;

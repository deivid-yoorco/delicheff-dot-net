...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// <param name="customerPointService">Custom number formatter</param>
        ...
        protected class PlaceOrderContainer
        {
            ...
            /// <summary>
            /// Total reward points required
            /// </summary>
            public decimal TotalRewardPointsRequired { get; set; }

            /// <summary>
            /// Total balance used on the order
            /// </summary>
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest, List<int> shoppingCartItemIds = null)
        {
            ...
            if (shoppingCartItemIds != null)
                details.Cart = details.Cart.Where(x => shoppingCartItemIds.Contains(x.Id)).ToList();
            ...
            var shouldValidate = (ShouldCheckForOrderMinimumValidation(details.Customer)
                        && _workContext.OriginalCustomerIfImpersonated == null
                        && TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea) ||
                        TeedCommerceStores.CurrentStore != TeedStores.CentralEnLinea;
            if (shouldValidate && !ValidateMinOrderSubtotalAmount(details.Cart))
            {
            ...
            if (shouldValidate && !ValidateMinOrderTotalAmount(details.Cart))
            {
            ...
            //sub total (incl tax)
            _orderTotalCalculationService.GetShoppingCartSubTotal(details.Cart, true, out decimal orderSubTotalDiscountAmount, out List<DiscountForCaching> orderSubTotalAppliedDiscounts, out decimal subTotalWithoutDiscountBase, out decimal _, out decimal requiredPoints);
            details.OrderSubTotalInclTax = subTotalWithoutDiscountBase;
            details.OrderSubTotalDiscountInclTax = orderSubTotalDiscountAmount;
            details.TotalRewardPointsRequired = requiredPoints;
            ...
            _orderTotalCalculationService.GetShoppingCartSubTotal(details.Cart, false, out orderSubTotalDiscountAmount,
                out orderSubTotalAppliedDiscounts, out subTotalWithoutDiscountBase, out _, out decimal requiredPoints_);
            ...
            if (orderShippingTotalInclTax == orderShippingTotalExclTax &&
                _taxSettings.ShippingIsTaxable && _taxSettings.ShippingPriceIncludesTax)
            {
                var rate =
                ...
                orderShippingTotalExclTax = orderShippingTotalInclTax / (1 + (rate / 100));
            }
            ...
            //balance used for current order (returns 0 if not used). Disable if no balance is left
            details.TotalBalanceAmount = _orderTotalCalculationService.GetBalanceTotalByOrderTotalOrCart(details.Customer, null, details.Cart);
            if (!(_orderTotalCalculationService.GetBalanceTotal(details.Customer) > 0))
            {
                details.Customer.IsPaymentWithBalanceActive = false;
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            var order = new Order
            {
                ...
                OriginalOrderTotal = details.OrderTotal,
                ...
                SignsGift = processPaymentRequest.CustomValues["namesigns"].ToString(),
                MessageGift = processPaymentRequest.CustomValues["messagesigns"].ToString(),
                SelectedShippingDate = (DateTime)processPaymentRequest.CustomValues["selectedShippingDate"] == default(DateTime) ? null : (DateTime?)processPaymentRequest.CustomValues["selectedShippingDate"],
                SelectedShippingTime = processPaymentRequest.CustomValues["selectedShippingTime"].ToString(),
                TotalRewardPointsRequired = details.TotalRewardPointsRequired,
                CustomerBalanceUsedAmount = details.TotalBalanceAmount
            };
        ...
        protected virtual void SendNotificationsAndSaveNotes(Order order)
        {
            ...
            if (order.TotalRewardPointsRequired > 0)
            {
                AddOrderNote(order, $"Se descontaron {order.TotalRewardPointsRequired} puntos de la cuenta del usuario.");
            }
        }
        /// <summary>
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer)
        {
                ...
                && notifyCustomer)
            {
                ...
                if (growthHackingSettings != null && growthHackingSettings.IsActive)
                {
                    GenerateRewardCouponCode(order, growthHackingSettings);
                }
            }
        ...
        /// <summary>
        /// Activate grow hacking for customer
        /// </summary>
        /// <param name="customer"></param>
        /// <param name="growthHackingSettings"></param>
        /// <param name="order"></param>
        /// <param name="orderCompletedCustomerNotificationQueuedEmailId"></param>
        /// <param name="orderSubTotal"></param>
        public void ActivateGrowHackingForCustomer(Customer customer,
            GrowthHackingSettings growthHackingSettings,
            Order order = null,
            decimal orderSubTotal = 9999999)
        {
            var customerCode = _discountService.GetAllDiscounts(showHidden: true).Where(x => x.CustomerOwnerId == customer.Id).FirstOrDefault();
            if (customerCode == null && orderSubTotal >= growthHackingSettings.MinimumAmountToCreateFriendCode)
            {
                string newCode = GenerateNewDiscountCode();
                Discount discount = new Discount()
                {
                    DiscountAmount = growthHackingSettings.UserCodeCouponAmount,
                    CouponCode = newCode,
                    CustomerOwnerId = customer.Id,
                    DiscountLimitation = DiscountLimitationType.SoloPrimeraOrden,
                    IsCumulative = false,
                    UsePercentage = false,
                    DiscountLimitationId = (int)DiscountLimitationType.SoloPrimeraOrden,
                    ...
                    Name = $"C�digo amigo {customer.Email} ({customer.Id})",
                    RequiresCouponCode = true,
                    AppliedToSubCategories = false,
                    LimitationTimes = 0,
                    OrderMinimumAmount = growthHackingSettings.UserCodeCouponOrderMinimumAmount
                };
                _discountService.InsertDiscount(discount);

                if (order != null)
                    AddOrderNote(order, $"Se gener� el c�digo autom�tico del cliente: {newCode}");

                var newCodeCustomerNotificationQueuedEmailId = _workflowMessageService
                .SendCustomerNewShareCode(customer, order != null ? order.CustomerLanguageId :
                _languageService.GetAllLanguages().Where(x => x.Published).Select(x => x.Id).FirstOrDefault(), newCode, growthHackingSettings.RewardAmount.ToString("C"));
                if (newCodeCustomerNotificationQueuedEmailId > 0 && order != null)
                {
                    AddOrderNote(order, $"Se envi� un correo al usuario con su nuevo c�digo. Queued email identifier: {newCodeCustomerNotificationQueuedEmailId}.");
                }
            }
            else if (customerCode.EndDateUtc.HasValue)
            {
                customerCode.EndDateUtc = null;
                _discountService.UpdateDiscount(customerCode);
                _logger.Information($"Se reactivó el código de amigo del usuario {customer.Email} ({customer.Id})", null, _workContext.CurrentCustomer);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private string GenerateNewDiscountCode(int charactersCount = 5)
        {
            string newCode;
            do
            {
                newCode = GenerateRandomCode(charactersCount);
        ...
        {
            details.Cart = details.Cart.GroupBy(x => x.ProductId).SelectMany(x => x).ToList();
            var doneDiscounts = new List<DiscountAppliedTemp>();
            var specialPromotionItems = DiscountHelper.GetSpecialPromotionItems(details.Customer, details.Cart,
            ...
            foreach (var sc in details.Cart)
            {
                ...
                var scSubTotal = _priceCalculationService.GetSubTotal(sc, true, out decimal discountAmount,
                    out List<DiscountForCaching> scDiscounts, out int? _, out decimal rewardPointsRequired
                    , doneDiscounts, specialPromotionItems);
                if (specialPromotionItems != null)
                {
                    var itemsDiscounted = specialPromotionItems.Where(x => x.ProductId == sc.ProductId &&
                        x.AttributesXml == sc.AttributesXml).ToList();
                    foreach (var itemDiscounted in itemsDiscounted)
                        discountAmount += itemDiscounted.Price;
                }
                doneDiscounts.AddRange(DiscountHelper.GetListOfCouponsBeingUsed(scDiscounts));
                ...
                var orderItem = new OrderItem
                {
                    ...
                    FancyDesign = sc.FancyDesign,
                    Customizable = sc.Product.Customizable,
                    BuyingBySecondary = sc.BuyingBySecondary,
                    SelectedPropertyOption = sc.SelectedPropertyOption,
                    EquivalenceCoefficient = sc.Product.EquivalenceCoefficient,
                    WeightInterval = sc.Product.WeightInterval,
                    UnitRewardPointsRequired = sc.Product.RewardPointsRequired,
                    RewardPointsRequired = rewardPointsRequired,
                    IsRewardItem = sc.IsRewardItem
                };
                ...
                var previousStock = sc.Product.ManageInventoryMethod == ManageInventoryMethod.ManageStock ||
                    sc.Product.ManageInventoryMethod == ManageInventoryMethod.DontManageStock ? sc.Product.StockQuantity :
                    _productAttributeParser.FindProductAttributeCombination(sc.Product, sc.AttributesXml)?.StockQuantity;
                ...
                if (previousStock.HasValue)
                {
                    ...
                    {
                        CreatedOnUtc = DateTime.UtcNow,
                        NewStock = previousStock.Value - sc.Quantity,
                        OldStock = previousStock.Value,
        ...
        protected virtual ProcessPaymentResult GetProcessPaymentResult(ProcessPaymentRequest processPaymentRequest, PlaceOrderContainer details)
        {
            ...
            if (!skipPaymentWorkflow)
            {
                ...
                if (details.TotalRewardPointsRequired > 0)
                {
                    decimal balance = _customerPointService.GetCustomerPointsBalance(processPaymentRequest.CustomerId);
                    if (balance < details.TotalRewardPointsRequired)
                        throw new NopException("El usuario no tiene los puntos requeridos para canjear las recompensas.");
                }
        ...
        {
            if (details.AppliedDiscounts == null)
                details.AppliedDiscounts = new List<DiscountForCaching>();

            //Check coupons used that applied to products
           
            //var extraCouponCodes = details.Cart.Where(x => !string.IsNullOrEmpty(x.AppliedByCodeByCoupon))
                //.Select(x => x.AppliedByCodeByCoupon).Distinct().ToList();
          
            

            //Check empty coupon codes
            var usedCoupons = details.AppliedDiscounts.Where(x => !string.IsNullOrEmpty(x.CouponCode))
                .Select(x => x.CouponCode.ToLower()).ToList();
            var otherCouponCodes = order.Customer.ParseAppliedDiscountCouponCodes().Select(x => x.ToLower()).ToList();
            var containedCoupons = otherCouponCodes.Intersect(usedCoupons).ToList();
            otherCouponCodes = otherCouponCodes.Where(x => !containedCoupons.Contains(x)).ToList();
            foreach (var otherCouponCode in otherCouponCodes)
            {
                var discount = _discountService.GetAllDiscounts(couponCode: otherCouponCode).FirstOrDefault();
                if (discount != null)
                {
                    if ((!discount.UsePercentage && discount.DiscountAmount == 0) || (discount.UsePercentage && discount.DiscountPercentage == 0))
                        details.AppliedDiscounts.Add(discount.MapDiscount());
                }
                else
                    try
                    {
                        order.Customer.RemoveDiscountCouponCode(otherCouponCode);
                    }
                    catch (Exception e)
                    {
                        _ = e;
            ...
            //clear shopping cart
            details.Cart.Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart).ToList()
        ...
        /// <summary>
        /// Generate reward coupon code used when a friend is using the customer code
        /// </summary>
        /// <param name="order"></param>
        /// <param name="growthHackingSettings"></param>
        public virtual void GenerateRewardCouponCode(Order order, GrowthHackingSettings growthHackingSettings)
        {
            var alreadyGeneratedCoupon = _discountService.GetAllDiscounts().Where(x => x.RelatedOrderId == order.Id).Any();
            if (alreadyGeneratedCoupon) return;
            var customer = order.Customer;
            var customerCode = _discountService.GetAllDiscounts().Where(x => x.CustomerOwnerId == customer.Id).FirstOrDefault();
            ActivateGrowHackingForCustomer(customer, growthHackingSettings, order, order.OrderSubtotalInclTax);

            Discount friendDiscount = order.DiscountUsageHistory.Where(x => x.Discount.CustomerOwnerId > 0).Select(x => x.Discount).FirstOrDefault();
            if (friendDiscount != null)
            {
                Customer friendCustomer = _customerService.GetCustomerById(friendDiscount.CustomerOwnerId);
                string couponCode = GenerateNewDiscountCode(7);
                Discount rewardDiscount = new Discount()
                {
                    DiscountAmount = growthHackingSettings.RewardAmount,
                    ...
                    DiscountTypeId = (int)DiscountType.AssignedToOrderSubTotal,
                    IsPopUpDiscount = false,
                    Name = $"Código de recompensa por invitación {friendCustomer.Email} ({couponCode})",
                    RequiresCouponCode = true,
                    AppliedToSubCategories = false,
                    LimitationTimes = 1,
                    LimitedToCustomerEmail = friendCustomer.Email,
                    OrderMinimumAmount = growthHackingSettings.RewardOrderMinimumAmount,
                    RelatedOrderId = order.Id
                };
                if (growthHackingSettings.RewardValidDays > 0)
                {
                    rewardDiscount.EndDateUtc = DateTime.UtcNow.AddDays(growthHackingSettings.RewardValidDays);
                }
                _discountService.InsertDiscount(rewardDiscount);


                var couponConditions = new List<string>()
                        {
                            "Cup�n aplicable a todos los productos de Central en l�nea.",
                            "El cup�n es personal e intransferible.",
                            "Cup�n para uso �nico.",
                            "No acumulable con otras promociones.",
                            "Nos reservamos el derecho de cancelar tu pedido si detectamos mal uso del cup�n."
                        };
                if (growthHackingSettings.RewardValidDays > 0)
                {
                    couponConditions.Add($"Vigencia hasta el {rewardDiscount.EndDateUtc.Value.ToLocalTime().ToString("dd 'de' MMMM 'de' yyyy", new CultureInfo("es-MX"))}");
                }
                if (growthHackingSettings.RewardOrderMinimumAmount > 0)
                {
                    couponConditions.Add($"Compra m�nima {growthHackingSettings.RewardOrderMinimumAmount:C} MXN con cualquier m�todo de pago");
                }

                var newCouponCustomerNotificationQueuedEmailId = _workflowMessageService
                .SendCustomerNewCouponCode(friendCustomer,
                order.CustomerLanguageId,
                couponCode,
                $"Has ganado un cup�n por un monto {growthHackingSettings.RewardAmount:C} MXN de descuento debido a una compra completada por parte de {order.Customer.GetFullName()}.",
                couponConditions);

                if (newCouponCustomerNotificationQueuedEmailId > 0)
                {
                    AddOrderNote(order, $"Se envi� cup�n de recompensa por invitaci�n a {friendCustomer.Email} ({friendCustomer.Id}) con un monto de descuento de {growthHackingSettings.RewardAmount:C}. C�digo del cup�n: {couponCode}");
                }
            }
        }
        ...
        /// </summary>
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <param name="shoppingCartItemIds">List of valid shopping cart item ids (for mobile checkout)</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest, List<int> shoppingCartItemIds = null)
        {
            ...
            try
            {
                var selectedDate = (DateTime)processPaymentRequest.CustomValues["selectedShippingDate"] == default ? null :
                    (DateTime?)processPaymentRequest.CustomValues["selectedShippingDate"];
                var orderNote = (string)processPaymentRequest.CustomValues["orderNote"] == default ? "" :
                    (string)processPaymentRequest.CustomValues["orderNote"];

                if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea && selectedDate.HasValue && selectedDate.Value.DayOfWeek == DayOfWeek.Sunday)
                {
                    result.AddError(string.Format(_localizationService.GetResource("Checkout.PaymentError"), "No se permiten órdenes para el domingo, por favor selecciona otra fecha."));
                    return result;
                }
                else if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea && selectedDate.HasValue && selectedDate.Value == DateTime.Now.Date)
                {
                    result.AddError(string.Format(_localizationService.GetResource("Checkout.PaymentError"), "No se permiten órdenes para el mismo día, por favor selecciona otra fecha."));
                    return result;
                }
                else if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea && !string.IsNullOrEmpty(orderNote) && processPaymentRequest.PaymentMethodSystemName == "Payments.Visa")
                {
                    if (orderNote == null) orderNote = "";
                    if (orderNote.Contains("Pago con Payments.Visa registrado desde la app móvil. Resultado:"))
                        orderNote = orderNote.Replace("Pago con Payments.Visa registrado desde la app móvil. Resultado: ", "");
                    if (!orderNote.Contains("{\"_links\":{"))
                    {
                        result.AddError(string.Format(_localizationService.GetResource("Checkout.PaymentError"), "El proceso de pago no pudo ser procesado, por favor, intenta con otro de nuestros métodos de pago (ERROR CODE: CBRSRC-MT)."));
                        return result;
                    }
                }
                ...
                var details = PreparePlaceOrderDetails(processPaymentRequest, shoppingCartItemIds);
                ...
                if (processPaymentResult.Success)
                {
                   ...
                   // SaveDiscountUsageHistory(details, order);
                    ...
                    //discount customer level points
                    if (order.TotalRewardPointsRequired > 0)
                    {
                        _customerPointService.InsertCustomerPoint(new Core.Domain.Rewards.CustomerPoint()
                        {
                            Points = order.TotalRewardPointsRequired * -1,
                            Description = "Puntos canjeados en productos de recompensa. Order #" + order.CustomOrderNumber,
                    ...
                    //send to branch email and save ShippingBranchOrder
                    if (TeedCommerceStores.CurrentStore == TeedStores.Masa)
                    {
                        using (HttpClient client = new HttpClient())
                        {
                            ...
                            var branchResult = client.PostAsync(url + "/Admin/ShippingBranch/SetShippingBranchOrder", content).Result;
                            if (!branchResult.IsSuccessStatusCode)
                            {
                                var resultJson = branchResult.Content.ReadAsStringAsync().Result;
                                _logger.Error($"[ORDER NOTIFICATION EMAIL] Error creating ShippingBranchOrder and branch email notification.", new Exception(resultJson));
                            }
                        }
                    }
        ...
        public virtual void CancelOrder(Order order, bool notifyCustomer)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
                ...
                if (TeedCommerceStores.CurrentStore != TeedStores.Lamy)
                {
                    ...
                    if (previousStock.HasValue)
                    {
                        ...
                        {
                            CreatedOnUtc = DateTime.UtcNow,
                            NewStock = previousStock.Value + orderItem.Quantity,
                            OldStock = previousStock.Value,
                            Product = orderItem.Product.Name,
                            ProductId = orderItem.Product.Id,
                            SKU = orderItem.Product.Sku,
                    ...
                    if (orderItem.Product.StockQuantity > 0 && !orderItem.Product.Published && TeedCommerceStores.CurrentStore != TeedStores.CentralEnLinea)
                    {
                        orderItem.Product.Published = true;
                        orderItem.Product.VisibleIndividually = true;
                        _productService.UpdateProduct(orderItem.Product);
                    }
                }
            }
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
                if (orderItem.Product.GiftProductEnable) continue;
                _shoppingCartService.AddToCart(order.Customer, orderItem.Product,
                    ...
                    orderItem.Quantity, false, orderItem.FancyDesign, orderItem.BuyingBySecondary, orderItem.SelectedPropertyOption);
            }
        ...
        {
            cart = cart.Where(x => x.ShoppingCartType == ShoppingCartType.ShoppingCart).ToList();
            if (cart == null)
            ...
            {
                //subtotal
                _orderTotalCalculationService.GetShoppingCartSubTotal(cart, _orderSettings.MinOrderSubtotalAmountIncludingTax, out decimal _, out List<DiscountForCaching> _, out decimal subTotalWithoutDiscountBase, out decimal _, out decimal requiredPoints);
        ...
        /// <summary>
        /// Get message validation of minimum order sub-total amount
        /// </summary>
        /// <param name="cart">Shopping cart</param>
        /// <returns>Returns message of minimum order sub-total, if order surpasses this amount string will be returned as empty</returns>
        public virtual string GetMinOrderSubtotalAmountMessage(IList<ShoppingCartItem> cart)
        {
            cart = cart.Where(x => x.ShoppingCartType == ShoppingCartType.ShoppingCart).ToList();
            ...
            if (cart.Any() && _orderSettings.MinOrderSubtotalAmount > decimal.Zero)
            {
                ...
                if (subTotalWithoutDiscountBase < _orderSettings.MinOrderSubtotalAmount)
                {
                    ...
                    var minOrderSubtotalAmount = _currencyService.ConvertFromPrimaryStoreCurrency(_orderSettings.MinOrderSubtotalAmount, _workContext.WorkingCurrency);
                    var format = _localizationService.GetResource("Checkout.MinOrderSubtotalAmount");
                    if (format.Count(f => f == '{') > 1)
                        msg = string.Format(format,
                            _priceFormatter.FormatPrice(minOrderSubtotalAmount, true, false),
                            _priceFormatter.FormatPrice((minOrderSubtotalAmount - subtotalBase), true, false));
                    else
                        msg = string.Format(format,
                            _priceFormatter.FormatPrice((minOrderSubtotalAmount - subtotalBase), true, false));
                }
        ...
        /// <summary>
        /// Get message validation of minimum order total amount
        /// </summary>
        /// <param name="cart">Shopping cart</param>
        /// <returns>Returns message of minimum order total, if order surpasses this amount string will be returned as empty</returns>
        public virtual string GetMinOrderTotalAmountMessage(IList<ShoppingCartItem> cart)
        {
            cart = cart.Where(x => x.ShoppingCartType == ShoppingCartType.ShoppingCart).ToList();
            var msg = string.Empty;
            if (cart == null)
            ...
            if (cart.Any() && _orderSettings.MinOrderTotalAmount > decimal.Zero)
            {
                ...
                if (shoppingCartTotalBase.HasValue && shoppingCartTotalBase.Value < _orderSettings.MinOrderTotalAmount)
                {
                    var totalBase = shoppingCartTotalBase ?? 0;
                    var minOrderTotalAmount = _currencyService.ConvertFromPrimaryStoreCurrency(_orderSettings.MinOrderTotalAmount, _workContext.WorkingCurrency);
                    var format = _localizationService.GetResource("Checkout.MinOrderTotalAmount");
                    if (format.Count(f => f == '{') > 1)
                        msg = string.Format(format,
                            _priceFormatter.FormatPrice(minOrderTotalAmount, true, false),
                            _priceFormatter.FormatPrice((minOrderTotalAmount - totalBase), true, false));
                    else
                        msg = string.Format(format,
                            _priceFormatter.FormatPrice((minOrderTotalAmount - totalBase), true, false));
                }
        ...
        /// <summary>
        /// Checks if order minimum should be checked, takes inner statements in consideration
        /// </summary>
        /// <param name="customer">Customer to check orders by</param>
        /// <param name="dateForCheckUtc">Date to check from (should be in UTC)</param>
        /// <returns>Returns boolean, "true" when order minimum should be checked, "false" when not</returns>
        public virtual bool ShouldCheckForOrderMinimumValidation(Customer customer, DateTime? dateForCheckUtc = null)
        {
            var shouldBeChecked = false;
            var controlDate = dateForCheckUtc == null ? DateTime.UtcNow.AddDays(-7) : dateForCheckUtc.Value.Date;
            var ordersOfLastWeek = _orderService.GetAllOrdersQuery().Where(x => (x.OrderStatusId == 20 || x.OrderStatusId == 10) &&
            !(x.PaymentMethodSystemName == "Payments.PayPalStandard" && x.PaymentStatusId == 10) &&
            DbFunctions.TruncateTime(x.CreatedOnUtc) >= controlDate && x.CustomerId == customer.Id).ToList();

            if (!ordersOfLastWeek.Any())

...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        /// <param name="quantity">Quantity</param>
        /// <param name="automaticallyAddRequiredProductsIfEnabled">Automatically add required products if enabled</param>
        /// <param name="rewardItemFlag">See if the product comes from an apapacho</param>
        /// <returns>Warnings</returns>
        public virtual IList<string> AddToCart(Customer customer, Product product,
            ShoppingCartType shoppingCartType, int storeId, string attributesXml = null,
            decimal customerEnteredPrice = decimal.Zero,
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, bool automaticallyAddRequiredProductsIfEnabled = true, string fancy = null, bool buyingBySecondary = false, string selectedPropertyOption = null, bool rewardItemFlag = false)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            RewardItem rewardsItems = null;
            if (rewardItemFlag)
            {
                rewardsItems = _rewardItemService.GetRewardItemByProductId(product.Id);
                if (rewardsItems != null)
                {
                    quantity = rewardsItems.Quantity;
                }
                product.Price = 0;
            ...
            if (shoppingCartItem != null)
            {
                if (shoppingCartType == ShoppingCartType.ShoppingCart)
                {
                    ...
                    if (!warnings.Any())
                    {
                        ...
                        if (customerEnteredPrice > 0) shoppingCartItem.CustomerEnteredPrice = customerEnteredPrice;
                        _customerService.UpdateCustomer(customer);
                ...
                else if (shoppingCartType == ShoppingCartType.Wishlist)
                {
            ...
            else
            {
                ...
                if (!warnings.Any())
                {
                    ...
                    //// We validate again if the product is not already in cart
                    //bool productIsInCart = false;
                    //if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea && customer.HasShoppingCartItems)
                    //{
                    //    productIsInCart = customer.ShoppingCartItems.Where(x => x.ProductId == product.Id).Any();
                    //}
                    ...
                    shoppingCartItem = new ShoppingCartItem
                    {
                        ...
                        Quantity = shoppingCartType == ShoppingCartType.Wishlist ? 1 : quantity,
                        RentalStartDateUtc = rentalStartDate,
                        RentalEndDateUtc = rentalEndDate,
                        CreatedOnUtc = now,
                        UpdatedOnUtc = now,
                        FancyDesign = fancy,
                        BuyingBySecondary = rewardsItems != null ? rewardsItems.BuyingBySecondary : buyingBySecondary,
                        SelectedPropertyOption = rewardsItems != null ? rewardsItems.SelectedPropertyOption : selectedPropertyOption,
                        IsRewardItem = rewardItemFlag
                    };
        ...
        public virtual IList<string> UpdateShoppingCartItem(Customer customer,
            ...
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, bool resetCheckoutData = true, bool buyingBySecondary = false, string selectedPropertyOption = null)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            if (quantity > 0)
            {
                ...
                shoppingCartItem.UpdatedOnUtc = DateTime.UtcNow;
                shoppingCartItem.BuyingBySecondary = buyingBySecondary;
                shoppingCartItem.SelectedPropertyOption = selectedPropertyOption;
                _customerService.UpdateCustomer(customer);
        ...
        public virtual void AddGiftProduct(Customer customer, int storeId)
        {
            var giftProducts = _productService.GetGiftProducts();
            if (giftProducts.Count > 0)
            {
                ...
                _orderTotalCalculationService.GetShoppingCartSubTotal(shoppingCartItems, true, out decimal _, out List<DiscountForCaching> _, out decimal subTotalWithoutDiscountBase, out decimal _, out decimal requiredPoints);

                // We remove all gift products
                foreach (var item in shoppingCartItems.Where(x => x.Product.GiftProductEnable).ToList())
                {
                    DeleteShoppingCartItem(item, true, true);
                }

                var selectedGiftForUser = giftProducts
                    .Where(x => x.GiftProductMinimumAmount < subTotalWithoutDiscountBase)
                    .OrderByDescending(x => x.GiftProductMinimumAmount)
                    .FirstOrDefault();
                if (selectedGiftForUser == null) return;
                if (selectedGiftForUser.ManageInventoryMethod == ManageInventoryMethod.ManageStock && selectedGiftForUser.StockQuantity <= 0) 
                    return;
                var addToCartWarnings = AddToCart(customer: customer,
                                       product: selectedGiftForUser,
                                       shoppingCartType: ShoppingCartType.ShoppingCart,
                                       storeId: storeId,
                                       automaticallyAddRequiredProductsIfEnabled: false,
                                       quantity: selectedGiftForUser.NumberProducts);
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            for (var i = 0; i < fromCart.Count; i++)
            {
                ...
                AddToCart(toCustomer, sci.Product, sci.ShoppingCartType, sci.StoreId,
                    ...
                    sci.FancyDesign, sci.BuyingBySecondary, sci.SelectedPropertyOption);
            }
            for (var i = 0; i < fromCart.Count; i++)
            {
                var sci = fromCart[i];
        ...
        /// </summary>
        /// <param name="customer">Customer</param>
        /// <param name="cartItemsIdToRemove">Arrary whit cartItemsIds to delete</param>
        public virtual void DeleteMigrateShoppingCart(Customer customer, int[] cartItemsIdToRemove)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var cartItemsRemove = cart.Where(x => cartItemsIdToRemove.Contains(x.Id)).ToList();

            for (var i = 0; i < cartItemsRemove.Count; i++)
            {
                var sci = cartItemsRemove[i];
                DeleteShoppingCartItem(sci);
            }
        }
        #endregion

...
namespace Nop.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        /// <param name="checkNotPublic">Discount Public???</param>
        /// <param name="showHidden">A value indicating whether to show hidden records</param>
        /// <returns>Discounts</returns>
        public virtual IList<Discount> GetAllDiscounts(DiscountType? discountType = null,
            string couponCode = "", string discountName = "", bool checkNotPublic = false, bool showHidden = false)
        {
            ...
            {
                query = query.Where(d => d.CouponCode.ToUpper() == couponCode.ToUpper());
            }
        ...
        /// <summary>
        /// Get product identifiers to which a discount is applied
        /// </summary>
        /// <param name="discount">Discount</param>
        /// <param name="customer">Customer</param>
        /// <returns>Manufacturer identifiers</returns>
        public virtual IList<int> GetAppliedProductIds(DiscountForCaching discount, Customer customer)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));

            var discountId = discount.Id;
            var cacheKey = string.Format(DiscountEventConsumer.DISCOUNT_PRODUCT_IDS_MODEL_KEY,
                discountId,
                string.Join(",", customer.GetCustomerRoleIds()),
                _storeContext.CurrentStore.Id);
            var result = _cacheManager.Get(cacheKey, () =>
            {
                return _discountRepository.Table.Where(x => x.Id == discountId)
                    .SelectMany(x => x.AppliedToProducts.Select(c => c.Id))
                    .ToList();
            });
        ...
        {
            return DoValidationWork(discount, customer, couponCodesToValidate);
        }
        ...
        /// <param name="couponCodesToValidate">Coupon codes to validate</param>
        /// <returns>Discount validation result</returns>
        public virtual DiscountValidationResult ValidateDiscountWithoutLimitedCouponUsage(Discount discount, Customer customer, string[] couponCodesToValidate)
        {
            return DoValidationWork(discount.MapDiscount(), customer, couponCodesToValidate, false);
        }

        private DiscountValidationResult DoValidationWork(DiscountForCaching discount, Customer customer,
            string[] couponCodesToValidate, bool checkLimitedToCouponUsage = true)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));
            ...
            if (discount.CustomerOwnerId > 0 && !_growthHackingSettings.IsActive)
            {
                result.Errors = new List<string>() { "No puedes utilizar este cupón." };
                return result;
            }
            ...
            switch (discount.DiscountLimitation)
            {
                ...
                case DiscountLimitationType.NTimesPerCustomer:
                    ...
                    break;
                case DiscountLimitationType.SoloPrimeraOrden:
                    {
                        var customerOrdersCount = _orderService.GetAllOrdersQuery().Where(x => x.CustomerId == customer.Id && !x.Deleted && x.OrderStatusId != 40).Count();
                        if (customerOrdersCount > 0)
                        {
                            result.Errors = new List<string>() { "El cupón solo es válido para tu primera orden." };
                            return result;
                        }
                        break;
                    }
            ...
            if (!string.IsNullOrWhiteSpace(discount.LimitedToCustomerEmail))
            {
                var emails = discount.LimitedToCustomerEmail.Split(',').Select(x => x.Trim().ToUpper()).ToList();
                if (!emails.Contains(customer.Email?.Trim().ToUpper()))
                {
                    result.Errors = new List<string>() { "No puedes utilizar este cupón." };
                    return result;
                }
            ...
            if (checkLimitedToCouponUsage)
                try
                {
                    if (!string.IsNullOrEmpty(discount.CouponCode) &&
                            discount.RequiresCouponCode)
                    {
                        var exclusiveDiscountCoupons = new List<string>();
                        var couponsBeingUsed = customer.ParseAppliedDiscountCouponCodes()
                        .Where(x => x.Trim().ToUpper() != discount.CouponCode.Trim().ToUpper())
                        .Select(x => x.Trim().ToUpper())
                        .ToList();
                        if (couponsBeingUsed.Any())
                        {
                            var discountsBeingUsed = GetAllDiscounts()
                                .Where(x => couponsBeingUsed.Contains(x.CouponCode?.Trim().ToUpper()) &&
                                ...
                                .Select(x => new
                                {
                                    ...
                                    x.CouponCode
                                    /*
                                    ExclusiveUsageAgainstIds = !string.IsNullOrEmpty(x.ExclusiveUsageAgainstIds) ?
                                        x.ExclusiveUsageAgainstIds.Split(',').Select(y => int.Parse(y)).ToList() :
                                        new List<int>()
                                    */
                                })
                                .ToList();

                            var discountsExlusiveOnly = ""; //discountsBeingUsed.Where(x => x.ExclusiveUsageAgainstIds.Contains(0)).ToList();
                            var discountsExlusiveUse = ""; //discountsBeingUsed.Where(x => x.ExclusiveUsageAgainstIds.Contains(discount.Id)).ToList();
                            //if (discountsExlusiveOnly.Any())
                               // exclusiveDiscountCoupons.AddRange(discountsExlusiveOnly.Select(x => x.CouponCode));
                            //else if (discountsExlusiveUse.Any())
                               // exclusiveDiscountCoupons.AddRange(discountsExlusiveUse.Select(x => x.CouponCode));

                            if (!string.IsNullOrWhiteSpace(discount.ExclusiveUsageAgainstIds))
                            {
                                var discountIds = discount.ExclusiveUsageAgainstIds.Split(',').Select(x => int.Parse(x)).ToList();
                                var discountIdsExlusiveOnly = discountIds.Where(x => x == 0).ToList();
                                var discountsBeingUsedIds = discountsBeingUsed.Select(x => x.Id).ToList();
                                var currentDiscountExclusiveUseIds = discountIds.Where(x => discountsBeingUsedIds.Contains(x)).ToList();
                                if (discountIdsExlusiveOnly.Any() && discountsBeingUsed.Any())
                                {
                                    result.Errors = new List<string>() { "No puedes utilizar este cupón en conjunto con otros cupones." };
                                    return result;
                                }
                                if (currentDiscountExclusiveUseIds.Any())
                                    exclusiveDiscountCoupons.AddRange(discountsBeingUsed.Where(x => currentDiscountExclusiveUseIds.Contains(x.Id)).Select(x => x.CouponCode));
                            }
                        }

                        if (exclusiveDiscountCoupons.Any())
                        {
                            result.Errors = new List<string>() { "No puedes utilizar este cupón en conjunto con los siguientes cupones: " +
                            string.Join(", ", exclusiveDiscountCoupons) };
                            return result;
                        }
                    }
                }
        ...
        /// <param name="shouldConsiderOrderFiltering">Should consider order cancellation filtering</param>
        ...
        public virtual IPagedList<DiscountUsageHistory> GetAllDiscountUsageHistory(int? discountId = null,
            ...
            bool shouldConsiderOrderFiltering = true)
        {
            ...
            discountUsageHistory = discountUsageHistory
                .Where(historyRecord => historyRecord.Order != null &&
                !historyRecord.Order.Deleted);

            //ignore cancelled and invalid orders
            if (shouldConsiderOrderFiltering)
                discountUsageHistory = discountUsageHistory.Where(historyRecord => historyRecord.Order.OrderStatusId != 40 &&
                !(historyRecord.Order.PaymentMethodSystemName == "Payments.PayPalStandard" && historyRecord.Order.PaymentStatusId == 10));
        ...
        /// <summary>
        /// Gets all discount usage history records query
        /// </summary>
        /// <returns>Discount usage history records query</returns>
        public virtual IQueryable<DiscountUsageHistory> GetAllDiscountUsageHistoryQuery()
        {
            return _discountUsageHistoryRepository.Table;
        }

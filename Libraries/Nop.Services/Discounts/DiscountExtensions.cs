...
namespace Nop.Services.Discounts
{
    ...
    public static class DiscountExtensions
    {
            ...
            List<DiscountAppliedTemp> doneDiscounts = null)
        {
            ...
            //substract amount discounted to current so global discount doesn't over reach original value 
            if (doneDiscounts != null)
            {
                var sameAsCurrentDiscount = doneDiscounts.Where(x => x.DiscountId == discount.Id).ToList();
                if (sameAsCurrentDiscount.Any())
                {
                    var discountAlreadyApplied = sameAsCurrentDiscount.Select(x => x.AmountDiscounted).DefaultIfEmpty().Sum();
                    var maxDiscount = discount.MaximumDiscountAmount.HasValue ? discount.MaximumDiscountAmount.Value : 0;
                    if (maxDiscount > 0)
                    {
                        var discountAvailable = maxDiscount - discountAlreadyApplied;
                        if (discountAvailable <= 0)
                        {
                            return 0;
                        }
                        else if (discountAvailable < result)
                        {
                            result = discountAvailable;
                        }
                    }
            ...
            List<DiscountAppliedTemp> doneDiscounts = null)
        {
            ...
            {
                var currentDiscountValue = discount.GetDiscountAmount(amount, doneDiscounts);
                if (currentDiscountValue > discountAmount)
                {
                    discountAmount = currentDiscountValue;
                    discount.DiscountedAmount = currentDiscountValue;
            ...
            {
                var cumulativeDiscountAmount = cumulativeDiscounts.Sum(d => d.GetDiscountAmount(amount, doneDiscounts));
        ...
        public static DiscountForCaching MapDiscount(this Discount discount)
        {
            ...
            return new DiscountForCaching
            {
                ...
                CustomerOwnerId = discount.CustomerOwnerId,
                LimitedToCustomerEmail = discount.LimitedToCustomerEmail,
                //ExclusiveUsageAgainstIds = discount.ExclusiveUsageAgainstIds,
                OrderMinimumAmount = discount.OrderMinimumAmount,
                //ShouldAddProducts = discount.ShouldAddProducts ?? false
            };
        }
    }
}

    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        protected Dictionary<string, IEnumerable<string>> AllowedTokens
        {
            get
            {
                ...
                _allowedTokens.Add(TokenGroupNames.StoreTokens, new[]
                {
                    ...
                    "%Email.HeaderImage4%",
                    "%Email.HeaderImage5%",
                    "%Email.ShippingImage1%",
                    "%Email.ShippingImage2%",
                    "%Email.ShippingImage3%",
                });
        ...
        protected virtual string ProductListToHtmlTable(Shipment shipment, int languageId)
        {
            ...
            for (var i = 0; i <= table.Count - 1; i++)
            {
                ...
                // picture with/without attributes
                var pictureUrl = _pictureService.GetPictureUrl(product.ProductPictures.FirstOrDefault().Picture);
                if (!string.IsNullOrEmpty(orderItem.AttributeDescription))
                {
                    var attributeValues = _productAttributeParser.ParseProductAttributeValues(orderItem.AttributesXml);
                    var attributeValueWithPicture = attributeValues.FirstOrDefault(x => x.PictureId > 0);
                ...
                string priceStr;
                if (orderItem.Order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax)
                {
                    //including tax
                    var priceInclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.PriceInclTax, orderItem.Order.CurrencyRate);
                    priceStr = _priceFormatter.FormatPrice(priceInclTaxInCustomerCurrency, true, orderItem.Order.CustomerCurrencyCode, language, true);
                }
                else
                {
                    //excluding tax
                    var priceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.PriceExclTax, orderItem.Order.CurrencyRate);
                    priceStr = _priceFormatter.FormatPrice(priceExclTaxInCustomerCurrency, true, orderItem.Order.CustomerCurrencyCode, language, false);
                }
                sb.AppendLine($"<td style=\"padding: 0.6em 0.4em;text-align: center;\">{priceStr}</td>");
        ...
        public virtual void AddStoreTokens(IList<Token> tokens, Store store, EmailAccount emailAccount)
        {
            ...
            var categories = allCategories.Where(c => c.ParentCategoryId == 0).ToList();
            var sb = new StringBuilder();
            sb.AppendLine("<ul id=\"horizontal-list\" style=\"text-align:center\">");
            foreach (var category in categories)
            {
                if (category.IncludeInTopMenu)
                {
                    sb.AppendLine($"<li><a href=\"{_webHelper.GetStoreHost(_webHelper.IsCurrentConnectionSecured())}{category.GetSeName()}\">{category.GetLocalized(x => x.Name)}</a></li>");
                }
            }
            sb.AppendLine("</ul>");
            var menu = sb.ToString();
            tokens.Add(new Token("Email.Menu", menu));

            HttpClient client = new HttpClient();

            var responseRaw = client.GetAsync(_webHelper.GetStoreHost(_webHelper.IsCurrentConnectionSecured()) + "EmailsPlugin/GetDataEmails").Result;
            if (responseRaw.IsSuccessStatusCode)
            {
                var responseJson = responseRaw.Content.ReadAsStringAsync().Result;

                var results = JsonConvert.DeserializeObject<StoreEmailModel>(responseJson);

                tokens.Add(new Token("Email.Address", results.Address));
                tokens.Add(new Token("Email.Aviso", results.Aviso));
                tokens.Add(new Token("Email.StoreColor", results.StoreColor));

                tokens.Add(new Token("Email.HeaderImage1", _pictureService.GetPictureUrl(results.HeaderEmailPicture1Id)));
                tokens.Add(new Token("Email.HeaderImage2", _pictureService.GetPictureUrl(results.HeaderEmailPicture2Id)));
                tokens.Add(new Token("Email.HeaderImage3", _pictureService.GetPictureUrl(results.HeaderEmailPicture3Id)));
                tokens.Add(new Token("Email.HeaderImage4", _pictureService.GetPictureUrl(results.HeaderEmailPicture4Id)));
                tokens.Add(new Token("Email.HeaderImage5", _pictureService.GetPictureUrl(results.HeaderEmailPicture5Id)));

                tokens.Add(new Token("Email.ShippingImage1", _pictureService.GetPictureUrl(results.ShippingEmailPicture1Id)));
                tokens.Add(new Token("Email.ShippingImage2", _pictureService.GetPictureUrl(results.ShippingEmailPicture2Id)));
                tokens.Add(new Token("Email.ShippingImage3", _pictureService.GetPictureUrl(results.ShippingEmailPicture3Id)));

                tokens.Add(new Token("Email.BtnTextWelcome", results.BtnTextWelcome));
                tokens.Add(new Token("Email.SubTitleWelcome", results.SubTitleWelcome));
                tokens.Add(new Token("Email.BtnTextPassword", results.BtnTextPassword));
                tokens.Add(new Token("Email.SubTitlePassword", results.SubTitlePassword));
                tokens.Add(new Token("Email.BtnTextStore", results.BtnTextStore));
                tokens.Add(new Token("Email.SubTitleStore", results.SubTitleStore));
                tokens.Add(new Token("Email.BtnTextPayment", results.BtnTextPayment));
                tokens.Add(new Token("Email.SubTitlePayment", results.SubTitlePayment));
                tokens.Add(new Token("Email.BtnTextPaypal", results.BtnTextPaypal));
                tokens.Add(new Token("Email.SubTitlePaypal", results.SubTitlePaypal));
                tokens.Add(new Token("Email.BtnTextOrderConfirm", results.BtnTextOrderConfirm));
                tokens.Add(new Token("Email.SubTitleOrderConfirm", results.SubTitleOrderConfirm));
                tokens.Add(new Token("Email.BtnTextOrderSent", results.BtnTextOrderSent));
                tokens.Add(new Token("Email.SubTitleOrderSent", results.SubTitleOrderSent));
                tokens.Add(new Token("Email.BtnTextOrderDelivered", results.BtnTextOrderDelivered));
                tokens.Add(new Token("Email.SubTitleOrderDelivered", results.SubTitleOrderDelivered));
            }
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            ...
            tokens.Add(new Token("Order.Total", "$" + order.OrderTotal));
            ...
            tokens.Add(new Token("Order.SelectedShippingDate", order.SelectedShippingDate.HasValue ?
                order.SelectedShippingDate.Value.ToString("dddd, dd 'de' MMMM 'de' yyyy", new CultureInfo("es-MX")) :
                string.Empty, true));
            tokens.Add(new Token("Order.SelectedShippingTime", order.SelectedShippingTime, true));
        ...
        public virtual void AddShipmentTokens(IList<Token> tokens, Shipment shipment, int languageId)
        {
            ...
            tokens.Add(new Token("Shipment.TrackingNumberURL", shipment.AdminComment));
        ...
        /// <param name="discountCode">Customer discount code</param>
        public virtual void AddCustomerTokens(IList<Token> tokens, Customer customer, string discountCode = null)
        {
            ...
            tokens.Add(new Token("Customer.NewShareCode", discountCode));

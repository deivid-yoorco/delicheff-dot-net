...
namespace Nop.Services.Messages
{
    ...
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        /// <summary>
        /// Send new code customer email
        /// </summary>
        /// <param name="customer"></param>
        /// <param name="languageId"></param>
        /// <param name="discountCode"></param>
        /// <returns></returns>
        public virtual int SendCustomerNewShareCode(Customer customer, int languageId, string discountCode, string rewardAmount)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var messageTemplate = GetActiveMessageTemplate(MessageTemplateSystemNames.CustomerNewShareCode, store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddCustomerTokens(tokens, customer, discountCode);
            tokens.Add(new Token("Discount.Amount", rewardAmount));
        ...
        /// <summary>
        /// Send new coupon code notification
        /// </summary>
        /// <param name="customer"></param>
        /// <param name="languageId"></param>
        /// <param name="couponCode"></param>
        /// <param name="couponReason"></param>
        /// <param name="couponConditions"></param>
        /// <returns></returns>
        public virtual int SendCustomerNewCouponCode(Customer customer, int languageId, string couponCode, string couponReason, List<string> couponConditions)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var messageTemplate = GetActiveMessageTemplate(MessageTemplateSystemNames.CustomerNewCouponCode, store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddCustomerTokens(tokens, customer);
            tokens.Add(new Token("Discount.Code", couponCode));
            tokens.Add(new Token("Discount.DiscountReason", couponReason));
            tokens.Add(new Token("Discount.DiscountConditions", CouponConditionListToHtml(couponConditions)));
        ...
        /// <param name="couponConditions"></param>
        /// <returns></returns>
        private string CouponConditionListToHtml(List<string> couponConditions)
        {
            string conditions = string.Empty;
            foreach (var condition in couponConditions)
                conditions += "<span>- " + condition + "</span><br />";
            return conditions;
        }
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendCustomerWelcomeMessage(Customer customer, int languageId, int discountId = 0)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            if (discountId != 0)
            {
                ...
                tokens.Add(new Token("Discount.Bool", discount.IsPopUpDiscount));
                tokens.Add(new Token("Discount.Code", discount.CouponCode));
        ...
        /// <param name="order">Order instance</param>
        /// <param name="languageId">Message language identifier</param>
        /// <param name="branchEmail">Optional branch send email</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendOrderPlacedStoreOwnerNotification(Order order, int languageId, string branchEmail = null)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var toEmail = string.IsNullOrEmpty(branchEmail) ? emailAccount.Email : branchEmail;
            var toName = emailAccount.DisplayName;

            return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                sendToBcc: string.IsNullOrEmpty(branchEmail));
        }
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            SendPushNotification(order.CustomerId, order.CustomOrderNumber, "order-paid");

            return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                attachmentFilePath, attachmentFileName);
        }

        private void SendPushNotification(int customerId, string orderNumber, string transactionType)
        {
            if (TeedCommerceStores.CurrentStore != TeedStores.CentralEnLinea) return;
            using (HttpClient client = new HttpClient())
            {
                ...
                var result = client.PostAsync(_storeContext.CurrentStore.SecureUrl + "/Api/Notification/CreateOrderTransactionNotification", content).Result;
                if (!result.IsSuccessStatusCode)
                {
                    var resultJson = result.Content.ReadAsStringAsync().Result;
                    _logger.Error($"[ORDER NOTIFICATION EMAIL] Error creating order transaction of type {transactionType} push notification.", new Exception(resultJson));
                }
            }
        }
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            SendPushNotification(order.CustomerId, order.CustomOrderNumber, "order-placed");
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            SendPushNotification(order.CustomerId, order.CustomOrderNumber, "order-completed");
        ...
        /// <param name="sendToBcc">Send to BCC mail</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendNotification(MessageTemplate messageTemplate,
            ...
            string fromEmail = null, string fromName = null, string subject = null, bool sendToBcc = true)
        {
            if (messageTemplate == null)
            ...
            var bcc = !sendToBcc ? null : messageTemplate.GetLocalized(mt => mt.BccEmailAddresses, languageId);
            ...
            var email = new QueuedEmail
            {
                ...
                UsedMessageTemplateId = messageTemplate.Id
            };

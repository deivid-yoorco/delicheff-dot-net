...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IQueryable<Customer> GetAllCustomersQuery()
        {
        ...
        /// <param name="street">Street; null to load all customers</param>
        /// <param name="colony">Colony; null to load all customers</param>
        ...
        public virtual IPagedList<Customer> GetAllCustomers(DateTime? createdFromUtc = null,
            ...
            string phone = null, string street = null, string colony = null, string zipPostalCode = null,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (!string.IsNullOrWhiteSpace(phone) && rg.Matches(phone.Trim()).Count > 0)
            {
                string customerIdsWithNumberQueryString =
                    @"SELECT EntityId 
                        FROM [dbo].[GenericAttribute] 
                        where KeyGroup = 'Customer' 
                        AND [Key] = 'Phone'
                        AND [Value] = '" + phone.Trim() + "'";
                List<int> customerIdsWithPhoneNumber = _dbContext.SqlQuery<int>(customerIdsWithNumberQueryString).ToList();
                query = query.Where(x => x.Addresses.Where(y => y.PhoneNumber.Contains(phone)).Any() || customerIdsWithPhoneNumber.Contains(x.Id));
            }

            //search by street
            if (!string.IsNullOrWhiteSpace(street))
            {
                query = query.Where(x => x.Addresses.Where(y => y.Address1.ToLower().Contains(street.ToLower())).Any());
            }

            //search by colony
            if (!string.IsNullOrWhiteSpace(colony))
            {
                query = query.Where(x => x.Addresses.Where(y => y.Address2.ToLower().Contains(colony.ToLower())).Any());
            }

            //search by zip
            if (!string.IsNullOrWhiteSpace(zipPostalCode))
            {
                query = query.Where(x => x.Addresses.Where(y => y.ZipPostalCode.ToLower().Contains(zipPostalCode.ToLower())).Any());
            }

...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CategoryController : BaseAdminController
    {
        ...
        public async Task<IActionResult> Create()
        {
            ...
            //if (model.Id != 0)
            //{
            //    model.ProductCatalogId = await GetProductCatalog(model.Id);
            //}
        ...
        public async Task<IActionResult> Create(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //if (!string.IsNullOrWhiteSpace(model.ProductCatalogId))
                //{
                //    await SaveProductCatalog(category.Id, model.ProductCatalogId);
                //}
        ...
        private async Task SaveProductCatalog(int categoryId, string productCatalogId)
        {
            using (HttpClient client = new HttpClient())
            {
                string url = (Request.IsHttps ? "https" : "http") + $"://{Request.Host}/Admin/Facturify/SaveCategoryProductCatalog?categoryId={categoryId}&productCatalogId={productCatalogId}";
                await client.GetAsync(url);
            }
        }


        //private async Task<string> GetProductCatalog(int categoryId)
        //{
        //    using (HttpClient client = new HttpClient())
        //    {
        //        string url = (Request.IsHttps ? "https" : "http") + $"://{Request.Host}/Admin/Facturify/GetCategoryProductCatalog?categoryId={categoryId}";
        ...
        //[HttpGet]
        //public IActionResult DeleteDuplicates()
        //{
        //    var allCategories = _categoryService.GetAllCategories(showHidden: true).ToList();
        //    var duplicatedGroup = allCategories.GroupBy(x => x.Name).Where(x => x.Count() > 1).ToList();
        //    foreach (var group in duplicatedGroup)
        //    {
        //        var test = group.OrderByDescending(x => x.CreatedOnUtc);
        //        var test2 = test.Take(test.Count() - 1).ToList();

    ...
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual void PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties)
        {
            ...
            if (customer != null)
            {
                ...
                model.IsGrowthHackingActive = _discountService.GetAllDiscounts().Where(x => x.CustomerOwnerId == customer.Id && !x.EndDateUtc.HasValue).Any();
                model.CheckoutSecurityIsDisabled = customer.CheckoutSecurityIsDisabled;
            }
            ...
            model.CurrentAdminIsBalanceManager = _permissionService.Authorize(StandardPermissionProvider.BalanceManager);
        }
        ...
        public virtual IActionResult ActivateGrowthHacking(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
            ...
            _orderProcessingService.ActivateGrowHackingForCustomer(customer, growthHackingSettings);
            _customerActivityService.InsertActivity("EditCustomer", _localizationService.GetResource("ActivityLog.EditCustomer") + " Se habilitó Growth Hacking", customer.Id);

            return RedirectToAction("Edit", new { id = customerId });
        }

        public virtual IActionResult DesactivateGrowthHacking(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedKendoGridJson();
            ...
            {
                customerCode.EndDateUtc = DateTime.UtcNow;
                _discountService.UpdateDiscount(customerCode);

                _customerActivityService.InsertActivity("EditCustomer", _localizationService.GetResource("ActivityLog.EditCustomer") + " Se deshabilitó Growth Hacking", customer.Id);
            }

            return RedirectToAction("Edit", new { id = customerId });
        }

        public virtual IActionResult DisableCheckoutSecurity(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedKendoGridJson();
            ...
            customer.CheckoutSecurityIsDisabled = true;

            _customerService.UpdateCustomer(customer);
            _customerActivityService.InsertActivity("EditCustomer", _localizationService.GetResource("ActivityLog.EditCustomer") + " Se desactivó la seguridad de checkout", customer.Id);

            return RedirectToAction("Edit", new { id = customerId });
        }

        public virtual IActionResult EnableCheckoutSecurity(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedKendoGridJson();
            ...
            _customerService.UpdateCustomer(customer);
            _customerActivityService.InsertActivity("EditCustomer", _localizationService.GetResource("ActivityLog.EditCustomer") + " Se reactivó la seguridad de checkout", customer.Id);
        ...
        public virtual IActionResult CustomerList(DataSourceRequest command, CustomerListModel model, int[] searchCustomerRoleIds)
        {
            ...
            var customers = _customerService.GetAllCustomers(
                ...
                street: model.SearchStreet,
                colony: model.SearchColony,
        ...
        [HttpGet]
        public IActionResult GetCustomerOrdersSelect(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.BalanceManager))
                return AccessDeniedView();

            var orderIdsOfCustomer = _orderService.GetAllOrdersQuery()
                .Where(x => !x.Deleted && x.CustomerId == customerId)
                .Select(x => x.Id)
                .OrderByDescending(x => x)
                .ToList();
            return Json(orderIdsOfCustomer);
        }

        [HttpPost]
        public IActionResult CustomerBalance(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();

            var customerBalance = _customerBalanceService.GetAllCustomerBalanceByCustomerId(customerId)
                .OrderByDescending(x => x.CreatedOnUtc)
                .ToList();
            var customerIds = customerBalance.Select(x => x.GivenByCustomerId).ToList();
            var givenByCustomers = _customerService.GetAllCustomersQuery()
                .Where(x => customerIds.Contains(x.Id))
                .ToList();
            DataSourceResult gridModel = new DataSourceResult
            {
                Data = customerBalance.Select(x => new
                {
                    x.Id,
                    Amount = x.Amount.ToString(),
                    CreatedOn = x.CreatedOnUtc.ToLocalTime().ToString("dd/MM/yyyy"),
                    x.Comment,
                    Order = x.OrderId.HasValue ? "#" + x.OrderId.Value : "No asignada",
                    GivenByCustomer = givenByCustomers.Where(y => y.Id == x.GivenByCustomerId).FirstOrDefault() != null ?
                    $"{givenByCustomers.Where(y => y.Id == x.GivenByCustomerId).FirstOrDefault().GetFullName()} ({givenByCustomers.Where(y => y.Id == x.GivenByCustomerId).FirstOrDefault().Email})" :
                    "No encontrado"
                }),
                Total = customerBalance.Count()
            };
        ...
        [HttpGet]
        public IActionResult GetCurrentCustomerBalance(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            var curretnBalance = _orderTotalCalculationService.GetBalanceTotal(_customerService.GetCustomerById(customerId));
            return Json(curretnBalance.ToString("C"));
        }

        [HttpPost]
        public IActionResult AddBalance(CustomerBalanceModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.BalanceManager))
                return Unauthorized();
            ...
            var balance = new CustomerBalance
            {
                ...
                GivenByCustomerId = _workContext.CurrentCustomer.Id,
                OrderId = model.OrderId,
                Log = $"{DateTime.Now:dd-MM-yyyy hh:mm:ss tt} - El usuario {_workContext.CurrentCustomer.Email} ({_workContext.CurrentCustomer.Id}) creó un nuevo balance para el cliente con Id {model.CustomerId} de cantidad {model.Amount.ToString("C")}.\n"
            };
            _customerBalanceService.InsertCustomerBalance(balance);

            return Ok();
        }

        [HttpPost]
        public IActionResult DeleteBalance(CustomerBalanceModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.BalanceManager))
                return Unauthorized();

            var balance = _customerBalanceService.GetCustomerBalanceById(model.Id);
            if (balance != null)
            {
                balance.Log += $"{DateTime.Now:dd-MM-yyyy hh:mm:ss tt} - El usuario {_workContext.CurrentCustomer.Email} ({_workContext.CurrentCustomer.Id}) eliminó este balance del usuario con Id {balance.CustomerId}.\n";
                _customerBalanceService.DeleteCustomerBalance(balance);
            }
        ...
        public virtual IActionResult AddressDelete(int id, int customerId)
        {
            ...
            if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea)
                _customerActivityService.InsertActivity(_workContext.CurrentCustomer, "DeleteAddress", _localizationService.GetResource("ActivityLog.DeleteAddress"),
        ...
        public virtual IActionResult AddressEdit(CustomerAddressModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea)
                    _customerActivityService.InsertActivity(_workContext.CurrentCustomer, "EditAddress", _localizationService.GetResource("ActivityLog.EditAddress"),
                    new string[] { $"{customer.Email} ({customer.Id})", address.Id.ToString(), address.JoinAddress() });
        ...
        public virtual IActionResult SmsVerifications()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.SmsVerification))
                return AccessDeniedView();

            return View("SmsVerifications", new VerificationFilteringModel());
        }

        [HttpPost]
        public virtual IActionResult SmsVerificationsList(DataSourceRequest command, VerificationFilteringModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.SmsVerification))
                return AccessDeniedView();

            var query = _smsVerificationService.GetSmsVerificationsQuery()
                .Where(x => !x.Deleted);
            if (model.Verified == 1)
                query = query.Where(x => x.IsVerified);
            else if (model.Verified == 2)
                query = query.Where(x => !x.IsVerified);
            if (!string.IsNullOrEmpty(model.PhoneNumber))
            {
                var number = model.PhoneNumber.Trim();
                query = query.Where(x => x.PhoneNumber == number);
            }
            var pagedList = new PagedList<SmsVerification>(query.OrderByDescending(x => x.CreatedOnUtc), command.Page - 1, command.PageSize);
            var customerEmails = pagedList.Select(x => x.CustomerEmail).ToList();
            var customers = _customerService.GetAllCustomersQuery()
                .Where(x => customerEmails.Contains(x.Email)).ToList()
                .Select(x => new { x.Email, Name = x.GetFullName() + " " }).ToList();

            var gridModel = new DataSourceResult
            {
                Data = pagedList.Select(x =>
                {
                    var customer = customers.Where(y => y.Email == x.CustomerEmail).FirstOrDefault();
                    return new
                    {
                        x.Id,
                        x.CustomerId,
                        Customer = (customer != null ? customer.Name : "Aún no registrado ") + $"({x.CustomerEmail})",
                        CreatedOn = x.CreatedOnUtc.ToLocalTime().ToString("dd/MM/yyyy hh:ss:mm tt"),
                        x.PhoneNumber,
                        x.VerificationCode,
                        x.IsVerified,
                        x.Log

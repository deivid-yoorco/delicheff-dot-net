...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class LanguageController : BaseAdminController
    {
        ...
        public virtual IActionResult ExportXls(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageLanguages))
                return AccessDeniedView();
            ...
            try
            {
                var xlsBytes = _localizationService.ExportResourcesToXls(language);
                return File(xlsBytes, MimeTypes.TextXlsx, $"language_pack.xlsx");
            }
            catch (Exception exc)
            {
                ErrorNotification(exc);
        ...
        [HttpPost]
        public virtual IActionResult ImportXls(int id, IFormFile importxlsfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageLanguages))
                return AccessDeniedView();
            ...
            try
            {
                if (importxlsfile != null && importxlsfile.Length > 0)
                {
                    _localizationService.ImportResourcesFromXls(language, importxlsfile);
                }
                else
                {
                    ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));

...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ManufacturerController : BaseAdminController
    {
        ...
        private readonly IManufacturerLogService _manufacturerLogService;
        ...
        public ManufacturerController(ICategoryService categoryService, 
            ...
            IManufacturerLogService manufacturerLogService)

        {
            ...
            this._manufacturerLogService = manufacturerLogService;
        ...
        public virtual void PrepareManufacturerLog(Manufacturer manufacturer, ManufacturerModel model)
        {
            ...
            logString += "Fabricante modificado por " + _workContext.CurrentCustomer.GetFullName() + " (" + userId + ") ";

            #region verification

            if (model.AllowCustomersToSelectPageSize != manufacturer.AllowCustomersToSelectPageSize)
            {
                logString += $"Editó permitir a los clientes elegir el tamaño de la página de {manufacturer.AllowCustomersToSelectPageSize} a {model.AllowCustomersToSelectPageSize}. ";
            }
            if (model.Description != manufacturer.Description)
            {
                logString += $"Editó la descripción. ";
            }
            if (model.DisplayDescription != manufacturer.DisplayDescription)
            {
                logString += $"Editó mostrar la descripción de {manufacturer.DisplayDescription} a {model.DisplayDescription}. ";
            }
            if (model.DisplayOrder != manufacturer.DisplayOrder)
            {
                logString += $"Editó mostrar orden de {manufacturer.DisplayOrder} a {model.DisplayOrder}. ";
            }
            if (model.DisplayPicture != manufacturer.DisplayPicture)
            {
                logString += $"Editó si la imagen puede ser mostrada de {manufacturer.DisplayPicture} a {model.DisplayPicture}. ";
            }
            if (model.DisplayTitle != manufacturer.DisplayTitle)
            {
                logString += $"Editó si puede mostrar el nombre de {manufacturer.DisplayTitle} a {model.DisplayTitle}. ";
            }
            if (model.MetaDescription != manufacturer.MetaDescription)
            {
                logString += $"Editó: Meta descripción. ";
            }
            if (model.MetaKeywords != manufacturer.MetaKeywords)
            {
                logString += $"Editó: Meta keywords. ";
            }
            if (model.MetaTitle != manufacturer.MetaTitle)
            {
                logString += $"Editó: Meta título de {manufacturer.MetaTitle} a {model.MetaTitle}. ";
            }
            if (model.Name != manufacturer.Name)
            {
                logString += $"Editó el nombre del fabricante de {manufacturer.Name} a {model.Name}. ";
            }
            if (model.PageSize != manufacturer.PageSize)
            {
                logString += $"Editó el tamaño de la página	. De {manufacturer.PageSize} a {model.PageSize}. ";
            }
            if (model.PageSizeOptions != manufacturer.PageSizeOptions)
            {
                logString += $"Editó las opciones para el tamaño de la página de {manufacturer.PageSizeOptions} a {model.PageSizeOptions}. ";
            }
            if (model.PictureId != manufacturer.PictureId)
            {
                logString += $"Editó el Id de la imagen de {manufacturer.PictureId} a {model.PictureId}. ";
            }
            if (model.PriceRanges != manufacturer.PriceRanges)
            {
                logString += $"Editó el rango de precios de {manufacturer.PriceRanges} a {model.PriceRanges}. ";
            }
            if (model.Published != manufacturer.Published)
            {
                logString += $"Editó si esta publicado de {manufacturer.Published} a {model.Published}. ";
            }
            if (model.IsIncludeInReportByManufacturer != manufacturer.IsIncludeInReportByManufacturer)
            {
                logString += $"Editó si está incluído en el reporte de fabricantes de {manufacturer.IsIncludeInReportByManufacturer} a {model.IsIncludeInReportByManufacturer}";
            }
            if (model.IsPaymentWhithCorporateCard != manufacturer.IsPaymentWhithCorporateCard)
            {
                logString += $"Editó si se paga con tarjeta corportativa de {manufacturer.IsPaymentWhithCorporateCard} a {model.IsPaymentWhithCorporateCard}";
            }
            if (model.IsPaymentWhithTransfer != manufacturer.IsPaymentWhithTransfer)
            {
                logString += $"Editó si se paga con transferencia de {manufacturer.IsPaymentWhithTransfer} a {model.IsPaymentWhithTransfer}";
            }

            #endregion

            var manufacturerLog = new ManufacturerLog()
            {
            ...
            _manufacturerLogService.InsertManufacturerLog(manufacturerLog);
        }
        ...
        [HttpGet]
        public List<int> GetProductsCountOfManufacturers(string ids)
        {
            ...
            foreach (var id in ids.Split(';'))
            {
                ...
                {
                    counts.Add(_manufacturerService.GetProductManufacturersByManufacturerId(Int32.Parse(id), showHidden: true).TotalCount);
                }
            }
        ...
        public virtual IActionResult Edit(int id)
        {
            ...
            model.ManufacturerLog = _manufacturerLogService.GetManufacturerLogById(manufacturer.Id);
        ...
        public virtual IActionResult Edit(ManufacturerModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                PrepareManufacturerLog(manufacturer, model);

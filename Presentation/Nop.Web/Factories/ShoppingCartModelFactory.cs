...
namespace Nop.Web.Factories
{
    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        private readonly RewardSettings _rewardSettings;
            ...
            CustomerSettings customerSettings)
        {
            ...
            this._rewardSettings = rewardSettings;
        ...
        protected virtual ShoppingCartModel.ShoppingCartItemModel PrepareShoppingCartItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci,
            IList<ShoppingCartItemModel> doneItems, List<DiscountAppliedTemp> doneDiscounts = null, List<SpecialPromotionItem> specialPromotionItems = null)
        {
            if (cart == null)
                throw new ArgumentNullException(nameof(cart));
            ...
            var combination = _productAttributeParser.FindProductAttributeCombination(sci.Product, sci.AttributesXml);

            var sQty = combination != null ? combination.StockQuantity : sci.Product.StockQuantity;

            var cartItemModel = new ShoppingCartModel.ShoppingCartItemModel
            {
                Id = sci.Id,
                Sku = sci.Product.FormatSku(sci.AttributesXml, _productAttributeParser),
                Gtin = sci.Product.FormatGtin(sci.AttributesXml, _productAttributeParser),
                ProductId = sci.Product.Id,
                ProductName = sci.Product.GetLocalized(x => x.Name),
                ProductSeName = sci.Product.GetSeName(),
                Quantity = sci.Quantity,
                AttributeInfo = _productAttributeFormatter.FormatAttributes(sci.Product, sci.AttributesXml),
                Category = sci.Product.ProductCategories.FirstOrDefault()?.Category?.Name,
                StockQuantity = sQty,
                FancyDesign = sci.FancyDesign,
                EquivalenceCoefficient = sci.Product.EquivalenceCoefficient,
                CustomerEnteredPrice = _priceFormatter.FormatPrice(sci.CustomerEnteredPrice),
                BuyingBySecondary = sci.BuyingBySecondary,
                SelectedPropertyOption = sci.SelectedPropertyOption,
                WeightInterval = sci.Product.WeightInterval,
                RewardPointsRequired = sci.Product.RewardPointsRequired,
                IsExtraCartProduct = sci.Product.IsExtraCartProduct,
                ProductCategoryNames = sci.Product.ProductCategories.Select(x => x.Category?.Name).ToList(),
                ProductManufacturerNames = sci.Product.ProductManufacturers.Select(x => x.Manufacturer?.Name).ToList(),
                GiftProductEnable = sci.Product.GiftProductEnable,
                IsRewardItem = sci.IsRewardItem,
                //AppliedByCodeByCoupon = sci.AppliedByCodeByCoupon
            };
                ...
                (!_orderSettings.AllowAdminsToBuyCallForPriceProducts || _workContext.OriginalCustomerIfImpersonated == null))
            {
                ...
                cartItemModel.UnitpriceWhitoutDiscount = cartItemModel.UnitPrice;
            }
            else
            {
                cartItemModel.UnitpriceWhitoutDiscount = sci.Product.Price.ToString("C");
                ...
                cartItemModel.UnitPrice = sci.Product.RewardPointsRequired > 0 ? shoppingCartUnitPriceWithDiscountBase.ToString("N") : _priceFormatter.FormatPrice(shoppingCartUnitPriceWithDiscount);
            }
            //subtotal, discount
            if (sci.Product.CallForPrice &&
                //also check whether the current user is impersonated
            ...
            else
            {
                ...
                //sub total
                var shoppingCartItemSubTotalWithDiscountBase = _taxService.GetProductPrice(sci.Product, _priceCalculationService.GetSubTotal(sci, true,
                    out decimal shoppingCartItemDiscountBase, out List<DiscountForCaching> appliedDiscounts, out int? maximumDiscountQty, out decimal rewardPointsRequired, doneDiscounts, specialPromotionItems), out decimal taxRate);
                if (doneDiscounts != null)
                ...
                cartItemModel.SubTotal = sci.Product.RewardPointsRequired > 0 ? shoppingCartItemSubTotalWithDiscountBase.ToString("N") : _priceFormatter.FormatPrice(shoppingCartItemSubTotalWithDiscount);
            ...
            {
                var appliedSpecialPromotionItems = specialPromotionItems.Where(x => x.ProductId == sci.ProductId &&
                    x.AttributesXml == sci.AttributesXml).ToList();
                foreach (var specialPromotionItem in appliedSpecialPromotionItems)
                {
                    var specialPromotion = _specialDiscountTakeXPayYService.GetSpecialDiscountByDiscountId(specialPromotionItem.AppliedId);
                    if (specialPromotion != null)
                    {
                        if (!cartItemModel.AppliedSpecialPromotionIds.Where(x => x == specialPromotionItem.AppliedId).Any())
                        {
                            cartItemModel.AppliedSpecialPromotionIds.Add(specialPromotionItem.AppliedId);

                            var entityName = specialPromotion.EntityTypeId == (int)TakeXPayYEntityType.Product ?
                            _productService.GetProductById(specialPromotion.EntityId)?.Name :
                            specialPromotion.EntityTypeId == (int)TakeXPayYEntityType.Category ?
                            _categoryService.GetCategoryById(specialPromotion.EntityId)?.Name : string.Empty;

                            cartItemModel.AppliedSpecialPromotionText += (!string.IsNullOrEmpty(cartItemModel.AppliedSpecialPromotionText) ? "<br>" : string.Empty) +
                                $"{specialPromotion.TakeAmount}x{specialPromotion.PayAmount}{(!string.IsNullOrEmpty(entityName) ? " (" + entityName + ")" : "")}";
                            cartItemModel.SubTotalWithoutSpecialPromotions = _priceFormatter.FormatPrice(
                                _currencyService.ConvertFromPrimaryStoreCurrency(_taxService.GetProductPrice(sci.Product, _priceCalculationService.GetSubTotal(sci), out _),
        ...
        protected virtual WishlistModel.ShoppingCartItemModel PrepareWishlistItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
        {
            ...
            ProductAttributeCombination combination = _productAttributeParser.FindProductAttributeCombination(sci.Product, sci.AttributesXml);
            ...
            var cartItemModel = new WishlistModel.ShoppingCartItemModel
            {
                ...
                Quantity = sci.Quantity,
                AttributeInfo = _productAttributeFormatter.FormatAttributes(sci.Product, sci.AttributesXml),
                StockQuantity = combination != null ? combination.StockQuantity : sci.Product.StockQuantity,
                RewardPointsRequired = sci.Product.RewardPointsRequired
            };
            ...
            {
                //sub total
                var shoppingCartItemSubTotalWithDiscountBase = _taxService.GetProductPrice(sci.Product, _priceCalculationService.GetSubTotal(sci, true, out decimal shoppingCartItemDiscountBase, out List<DiscountForCaching> _, out int? maximumDiscountQty, out decimal rewardPointsRequired), out decimal taxRate);
                var shoppingCartItemSubTotalWithDiscount = _currencyService.ConvertFromPrimaryStoreCurrency(shoppingCartItemSubTotalWithDiscountBase, _workContext.WorkingCurrency);
                cartItemModel.SubTotal = sci.Product.RewardPointsRequired > 0 ? shoppingCartItemSubTotalWithDiscountBase.ToString("N") : _priceFormatter.FormatPrice(shoppingCartItemSubTotalWithDiscount);
                cartItemModel.MaximumDiscountedQty = maximumDiscountQty;
        ...
        /// <summary>
        /// Prepare extra shopping cart product model
        /// </summary>
        /// <param name="model"></param>
        /// <param name="cart"></param>
        /// <param name="extraCartProducts"></param>
        /// <param name="customer"></param>
        /// <param name="storeId"></param>
        /// <returns></returns>
        public virtual ExtraShoppingCartProductModel PrepareExtraShoppingCartProductModel(ExtraShoppingCartProductModel model,
            IList<ShoppingCartItem> cart, List<Product> extraCartProducts, Customer customer, int storeId)
        {
            if (cart == null)
                throw new ArgumentNullException(nameof(cart));
            ...
            if (extraCartProducts == null)
                throw new ArgumentNullException(nameof(extraCartProducts));

            //items
            foreach (var ecp in extraCartProducts)
            {
                var sci = cart.Where(x => x.ProductId == ecp.Id).FirstOrDefault();
                if (sci == null)
                {
                ...
                var cartItemModel = PrepareShoppingCartItemModel(cart, sci, model.Items);
            ...
            bool prepareAndDisplayOrderReviewData = false)
        {
            ...
            if ((_orderProcessingService.ShouldCheckForOrderMinimumValidation(_workContext.CurrentCustomer)
                    ...
                    TeedCommerceStores.CurrentStore != TeedStores.CentralEnLinea)
            {
                model.MinOrderSubtotalWarning = _orderProcessingService.GetMinOrderSubtotalAmountMessage(cart);
                model.MinOrderTotalWarning = _orderProcessingService.GetMinOrderTotalAmountMessage(cart);
            }
            ...
            //IsGift enable in admin
            model.ShowIsGift = _shoppingCartSettings.ShowIsGift;
            ...
            {
                var cartItemModel = PrepareShoppingCartItemModel(cart, sci, model.Items, doneDiscounts, specialPromotionItems);
                model.Items.Add(cartItemModel);
            }

            //Discount
            model.DiscountBox.DiscountViewList = _discountService.GetAllDiscounts().Where(x => x.DisplayToCustomer == true && x.DisplayToCustomer != null).ToList();
        ...
        public virtual MiniShoppingCartModel PrepareMiniShoppingCartModel()
        {
            ...
            if (_workContext.CurrentCustomer.HasShoppingCartItems)
            {
                ...
                if (cart.Any())
                {
                    ...
                    _orderTotalCalculationService.GetShoppingCartSubTotal(cart, subTotalIncludingTax, out decimal _, out List<DiscountForCaching> _, out decimal subTotalWithoutDiscountBase, out decimal _, out decimal requiredPoints, false);
                    ...
                    model.RequiredRewardsPointsTotal = requiredPoints;
                    model.GamificationIsActive = _rewardSettings.IsActive;
                        ...
                        .ToList())
                    {
                        var cartItemModel = new MiniShoppingCartModel.ShoppingCartItemModel
                        {
                            ...
                            BuyingBySecondary = sci.BuyingBySecondary,
                            WeightInterval = sci.Product.WeightInterval,
                            RequiredRewardsPoints = sci.Product.RewardPointsRequired
                        };
                        ...
                        {
                            var shoppingCartUnitPriceWithDiscountBase = _taxService.GetProductPrice(sci.Product, _priceCalculationService.GetUnitPrice(sci, false, out decimal _, out List<DiscountForCaching> appliedDiscounts, doneDiscounts), out decimal _);
                            doneDiscounts.AddRange(DiscountHelper.GetListOfCouponsBeingUsed(appliedDiscounts));
        ...
        public virtual OrderTotalsModel PrepareOrderTotalsModel(IList<ShoppingCartItem> cart, bool isEditable)
        {
            var model = new OrderTotalsModel
            {
                ...
                IsPaymentWithBlanceActive = _workContext.CurrentCustomer.IsPaymentWithBalanceActive ?? false
            };

            if (cart.Any())
            {
                // Product discounts
                model.ProductDiscountsValue = 0;
                var doneDiscounts = new List<DiscountAppliedTemp>();
                var specialPromotionItems = DiscountHelper.GetSpecialPromotionItems(cart.GetCustomer(), cart,
                    _discountService, _specialDiscountTakeXPayYService, _priceCalculationService);
                foreach (var sci in cart)
                {
                    _taxService.GetProductPrice(sci.Product, _priceCalculationService.GetSubTotal(sci, true, out decimal shoppingCartItemDiscountBase,
                        out List<DiscountForCaching> appliedDiscounts, out int? maximumDiscountQty, out decimal rewardPointsRequired, doneDiscounts, specialPromotionItems), out decimal taxRate);
                    doneDiscounts.AddRange(DiscountHelper.GetListOfCouponsBeingUsed(appliedDiscounts));
                    ...
                    if (shoppingCartItemDiscountBase > decimal.Zero)
                    {
                        ...
                        {
                            var shoppingCartItemDiscount = _currencyService.ConvertFromPrimaryStoreCurrency(shoppingCartItemDiscountBase, _workContext.WorkingCurrency);
                            model.ProductDiscountsValue += shoppingCartItemDiscount;
                        }
                    }
                }

                if (model.ProductDiscountsValue > 0)
                    model.ProductDiscounts = _priceFormatter.FormatPrice(-model.ProductDiscountsValue);

                //subtotal
                var subTotalIncludingTax = _workContext.TaxDisplayType == TaxDisplayType.IncludingTax && !_taxSettings.ForceTaxExclusionFromOrderSubtotal;
                _orderTotalCalculationService.GetShoppingCartSubTotal(cart, subTotalIncludingTax, out decimal orderSubTotalDiscountAmountBase, out List<DiscountForCaching> _, out decimal subTotalWithoutDiscountBase, out decimal _, out decimal requiredPoints);
                var subtotalBase = subTotalWithoutDiscountBase;
                var subtotal = _currencyService.ConvertFromPrimaryStoreCurrency(subtotalBase, _workContext.WorkingCurrency);
                model.SubtotalValue = subtotal;
                if (model.ProductDiscountsValue > 0)
                    model.SubtotalValue += model.ProductDiscountsValue;
                model.SubTotal = _priceFormatter.FormatPrice(model.SubtotalValue, true, _workContext.WorkingCurrency, _workContext.WorkingLanguage, subTotalIncludingTax);
                model.RequiredRewardPointsValue = requiredPoints;
                model.RequiredRewardPoints = requiredPoints.ToString("N");

                model.CustomerRewardPointsBalance = _customerPointService.GetCustomerPointsBalance(_workContext.CurrentCustomer.Id);
                ...
                if (orderSubTotalDiscountAmountBase > decimal.Zero)
                {
                    ...
                    model.SubTotalDiscountValue = orderSubTotalDiscountAmount;
                ...
                if (model.RequiresShipping)
                {
                    ...
                    if (shoppingCartShippingBase.HasValue)
                    {
                        ...
                        model.ShippingValue = shoppingCartShipping;
                ...
                if (orderTotalDiscountAmountBase > decimal.Zero)
                {
                    ...
                    model.OrderTotalDiscountValue = orderTotalDiscountAmount;
                ...
                //reward points totals
                RewardSettings settings = _settingService.LoadSetting<RewardSettings>();
                model.OrderTotalPoints = Math.Round(model.OrderTotalValue * settings.OrderPoints, 2);
                model.RewardsActive = settings.IsActive;

                //customer balance totals
                var shoppingCartTotalBaseWithouBalance = _orderTotalCalculationService.GetShoppingCartTotal(cart, out _, out _, out _, out _, out _, checkBalanceUsage: false);
                if (shoppingCartTotalBaseWithouBalance.HasValue)
                {
                    var shoppingCartTotal = _currencyService.ConvertFromPrimaryStoreCurrency(shoppingCartTotalBaseWithouBalance.Value, _workContext.WorkingCurrency);

                    var curretnCustomerBalance = _orderTotalCalculationService.GetBalanceTotal(cart.GetCustomer());
                    model.OrderTotalBalance = curretnCustomerBalance > shoppingCartTotal ? shoppingCartTotal : curretnCustomerBalance;
                }
            }
        ...
        public virtual List<ShoppingCartItem> CheckIfCartHasDeletedProducts(List<ShoppingCartItem> cart)
        {
            foreach (var sci in cart)
            {
                if (sci.Product.Deleted)
                {
                    Console.WriteLine(sci.Product.Name);

...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        /// 
        protected virtual ProductOverviewModel.ProductPriceModel PrepareProductOverviewPriceModel(Product product, List<DiscountAppliedTemp> doneDiscounts, bool forceRedirectionAfterAddingToCart = false)
        {
            if (product == null)
            ...
            switch (product.ProductType)
            {
                ...
                default:
                    {
                        //simple product
                        PrepareSimpleProductOverviewPriceModel(product, priceModel, doneDiscounts);
                    }
                    break;
            }
        ...
        protected virtual void PrepareSimpleProductOverviewPriceModel(Product product, ProductOverviewModel.ProductPriceModel priceModel, List<DiscountAppliedTemp> doneDiscounts)
        {
            //add to cart button
            ...
            priceModel.EquivalenceCoefficient = product.EquivalenceCoefficient;
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                decimal discountAmount = _priceCalculationService.GetDiscountAmount(product, _workContext.CurrentCustomer, product.Price, out List<DiscountForCaching> appliedDiscounts, doneDiscounts, false);
                doneDiscounts.AddRange(DiscountHelper.GetListOfCouponsBeingUsed(appliedDiscounts));

                if (discountAmount > 0)
                {
                    priceModel.PriceWithDiscountValue = product.Price - discountAmount;
                    priceModel.PriceWithDiscount = _priceFormatter.FormatPrice(priceModel.PriceWithDiscountValue);
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            priceModel.OldPrice = product.Price != finalPrice ? _priceFormatter.FormatPrice(product.Price) : null;
                            priceModel.Price = _priceFormatter.FormatPrice(finalPrice);
                            priceModel.PriceValue = finalPrice;
                        }
                    }
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        model.RewardPointsRequired = product.RewardPointsRequired;
        ...
        protected virtual ProductDetailsModel.AddToCartModel PrepareProductAddToCartModel(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            //Max quantity
            model.StockQty = product.StockQuantity;
            //allowed quantities
        ...
        protected virtual IList<ProductDetailsModel.ProductAttributeModel> PrepareProductAttributeModels(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            foreach (var attribute in productAttributeMapping)
            {
                var attributeModel = new ProductDetailsModel.ProductAttributeModel
                {
                    ...
                    BoundingX = product.Customizable ?
                    product.ProductPictures.Where(x => x.Picture.BoundingX != null).FirstOrDefault()?.Picture.BoundingX : null,
                    BoundingY = product.Customizable ?
                    product.ProductPictures.Where(x => x.Picture.BoundingY != null).FirstOrDefault()?.Picture.BoundingY : null,
                    BoundingWidth = product.Customizable ?
                    product.ProductPictures.Where(x => x.Picture.BoundingWidth != null).FirstOrDefault()?.Picture.BoundingWidth : null,
                    BoundingHeight = product.Customizable ?
                    product.ProductPictures.Where(x => x.Picture.BoundingHeight != null).FirstOrDefault()?.Picture.BoundingHeight : null
                };
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        var valueModel = new ProductDetailsModel.ProductAttributeValueModel
                        {
                            ...
                            StockQty = ProductUtils.GetProductStock(product, attributeValue.Id.ToString(), _productAttributeParser, _productAttributeService),
                            ...
                            ProductAwards = TeedCommerceStores.CurrentStore == TeedStores.Lamy ? attributeValue.ProductAwards : "",
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                foreach (var picture in pictures)
                {
                    var pictureModel = new PictureModel
                    {
                        ...
                        Is360 = picture.Is360,
                        CustomEnable = picture.CustomEnable,
                        BoundingX = picture.BoundingX,
                        BoundingY = picture.BoundingY,
                        BoundingWidth = picture.BoundingWidth,
                        BoundingHeight = picture.BoundingHeight
                    };
                    //"title" attribute
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                List<ProductAttributeValue> attributesModel = new List<ProductAttributeValue>();
                List<PictureModel> PictureModelsList = new List<PictureModel>();

                //var xmlAttr = _productAttributeParser.GenerateAllCombinations(product);

                foreach (var attribute in attributes)
                {
                    ProductAttributeValue addAttributesModel = new ProductAttributeValue();
                    var item = attribute.ProductAttributeValues.FirstOrDefault();

                    if (item != null)
                    {
                        addAttributesModel.AssociatedProductId = item.AssociatedProductId;
                        addAttributesModel.AttributeValueType = item.AttributeValueType;
                        addAttributesModel.AttributeValueTypeId = item.AttributeValueTypeId;
                        addAttributesModel.ColorSquaresRgb = item.ColorSquaresRgb;
                        addAttributesModel.Cost = item.Cost;
                        addAttributesModel.CustomerEntersQty = item.CustomerEntersQty;
                        addAttributesModel.DisplayOrder = item.DisplayOrder;
                        addAttributesModel.Id = item.Id;
                        addAttributesModel.ImageSquaresPictureId = item.ImageSquaresPictureId;
                        addAttributesModel.IsPreSelected = item.IsPreSelected;
                        addAttributesModel.Name = item.Name;
                        addAttributesModel.PictureId = item.PictureId;
                        addAttributesModel.PriceAdjustment = item.PriceAdjustment;
                        addAttributesModel.ProductAttributeMapping = item.ProductAttributeMapping;
                        addAttributesModel.ProductAttributeMappingId = item.ProductAttributeMappingId;
                        addAttributesModel.Quantity = item.Quantity;
                        addAttributesModel.WeightAdjustment = item.WeightAdjustment;
                        addAttributesModel.StockQty = item.StockQty;

                        foreach (var value in item.ProductAttributeMapping.ProductAttributeValues)
                        {
                            value.StockQty = ProductUtils.GetProductStock(product, value.Id.ToString(), _productAttributeParser, _productAttributeService);
                        }

                        attributesModel.Add(addAttributesModel);
                    }
                }
                ...
                var model = new ProductOverviewModel
                {
                    ...
                    AttributeProduct = attributesModel,
                    PictureModels = PictureModelsList,
                    Quantity = stock,
                    StockAvailability = product.DisplayStockAvailability,
                    CurrentQuantityInCart = _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.ProductId == product.Id && x.ShoppingCartType == ShoppingCartType.ShoppingCart).Select(x => x.Quantity).FirstOrDefault(),
                    BuyingBySecondary = _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.ProductId == product.Id && x.ShoppingCartType == ShoppingCartType.ShoppingCart).Select(x => x.BuyingBySecondary).FirstOrDefault() || _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.ProductId == product.Id).Select(x => x.Product.WeightInterval > 0).FirstOrDefault(),
                    Product = product,
                    PropertiesOptions = product.PropertiesOptions?.Split(',').ToList(),
                    SelectedPropertyOption = _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.ProductId == product.Id && x.ShoppingCartType == ShoppingCartType.ShoppingCart).Select(x => x.SelectedPropertyOption).FirstOrDefault(),
                    WeightInterval = product.WeightInterval,
                    IsInWishList = _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.ProductId == product.Id && x.ShoppingCartType == ShoppingCartType.Wishlist).Any()
                };

                //price
                if (preparePriceModel)
                {
                    model.ProductPrice = PrepareProductOverviewPriceModel(product, doneDiscounts, forceRedirectionAfterAddingToCart);
                }
                ...
                model.DefaultPictureModel = PrepareProductDetailsPictureModel(product, false, out IList<PictureModel> allPictureModels);
                model.PictureModels = allPictureModels.ToList();
                ...
                if (product.HasDiscountsApplied)
                {
                    ...
                    {
                        if (_discountService.ValidateDiscountWithoutLimitedCouponUsage(appliedDiscount, _workContext.CurrentCustomer, new string[1] { appliedDiscount.CouponCode }).IsValid &&
                           (appliedDiscount.DiscountType == DiscountType.AssignedToSkus || appliedDiscount.DiscountType == DiscountType.AssignedToCategories) &&
                           appliedDiscount.UsePercentage && !string.IsNullOrEmpty(appliedDiscount.CouponCode) &&
                           appliedDiscount.RequiresCouponCode) /*&& (appliedDiscount.IsExtraVisible ?? false))*/
                            discounts.Add(appliedDiscount);
                    }
                    if (discounts.Any())
                    {
                        var visibleDiscount = discounts.FirstOrDefault();
                        model.VisibleDiscount = new ProductOverviewModel.VisibleDiscountModel
                        {
                            Id = visibleDiscount.Id,
                            CouponCode = visibleDiscount.CouponCode,
                            DiscountAmount = visibleDiscount.DiscountPercentage
                        };
                    }
                }
                    ...
                    ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                ProductOverview = PrepareProductOverviewModels(list).FirstOrDefault(),
                Customizable = product.Customizable,
                ShippingText = product.ShippingText,
                ShippingTextEnable = product.ShippingTextEnable
            };
            ...
            model.IsReturnable = product.NotReturnable;

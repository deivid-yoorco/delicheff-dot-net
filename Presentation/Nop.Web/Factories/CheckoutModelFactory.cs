    ...
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        public virtual CheckoutShippingMethodModel PrepareShippingMethodModel(IList<ShoppingCartItem> cart, Address shippingAddress)
        {
            ...
            if (getShippingOptionResponse.Success)
            {
                ...
                foreach (var shippingOption in getShippingOptionResponse.ShippingOptions)
                {
                    ...
                    if (TeedCommerceStores.CurrentStore == TeedStores.Hamleys && _orderTotalCalculationService.IsFreeShippingByShippingOption(cart, shippingOption))
                        shippingTotal = 0;
        ...
        public virtual CheckoutCompletedModel PrepareCheckoutCompletedModel(Order order)
        {
            ...
            var model = new CheckoutCompletedModel
            {
                ...
                RewardsActive = settings.IsActive
            };
            model.OrderTotalPoints = Math.Round(order.OrderTotal * settings.OrderPoints, 2);
        ...
        public async virtual Task<OnePageCheckoutModel> PrepareOnePageCheckoutModel(IList<ShoppingCartItem> cart)
        {
            if (cart == null)
            ...
            var totalModel = _shoppingCartModelFactory.PrepareOrderTotalsModel(cart, false);

            var addressModel = new CustomerAddressEditModel();
            _addressModelFactory.PrepareAddressModel(addressModel.Address,
               address: null,
               excludeProperties: false,
               addressSettings: _addressSettings,
               loadCountries: () => _countryService.GetAllCountries(_workContext.WorkingLanguage.Id));

            var anyProductHasSpecificationOptionId = false;
            if (TeedCommerceStores.CurrentStore == TeedStores.Cetro)
            {
                anyProductHasSpecificationOptionId = cart.Select(x => x.Product)
            ...
            var model = new OnePageCheckoutModel
            {
                ...
                ShippingAddress = PrepareShippingAddressModel(),
                CartModel = cartModel,
                TotalModel = totalModel,
                Address = addressModel.Address,
                AllProductsNoShipping = !cart.Where(x => x.Product.IsShipEnabled).Any(),
                AnyProductHasTicketSpecification = anyProductHasSpecificationOptionId
            };

...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        protected virtual void PrepareFacturify(OrderDetailsModel model, Order order)
        {
            try
            {
                ...
                model.AllowGeneration = GetAllowGeneration().Result && order.OrderStatusId == 30 && !order.DiscountUsageHistory.Where(x => x.Discount.Name.ToLower().Contains("azteca")).Any();
            }
            catch (Exception e)
            {
                model.Bill = null;
                model.CustomerFacturifyAddresses = new List<CustomerFacturifyAddress>();
                model.AllowGeneration = false;
            }
        }
        ...
        protected async Task<bool> GetAllowGeneration()
        {
            using (HttpClient client = new HttpClient())
            {
                string url = (Request.IsHttps ? "https" : "https") + $"://{Request.Host}/Admin/Facturify/GetAllow";
                var result = await client.GetAsync(url);
                if (result.IsSuccessStatusCode)
                {
                    var json = await result.Content.ReadAsStringAsync();

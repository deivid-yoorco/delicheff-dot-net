...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public virtual IActionResult GetCategoriesModal(bool isHome)
        {
            List<Category> categories =
                isHome ? _categoryService.GetAllCategories().Where(x => x.ParentCategoryId == 0).ToList() :
                _categoryService.GetAllCategories().ToList();
            var json = new List<AllCategories>();
            foreach (var category in categories)
            {
                json.Add(new AllCategories
                {
                    Name = category.Name,
                    SeNames = category.GetSeName(),
                    ImgUrl = _pictureService.GetPictureUrl(category.PictureId)
                });
            }
            return Json(json);
        }
        ...
        {
            if (TeedCommerceStores.CurrentStore == TeedStores.CentralEnLinea) return RedirectToActionPermanent("Index", "Home");
        ...
        [HttpsRequirement(SslRequirement.No)]
        public virtual IActionResult Rewards(int manufacturerId, CatalogPagingFilteringModel command)
        {
            var settings = _rewardSettings;
            if (!settings.Active) return NotFound();

            //model
            var model = _catalogModelFactory.PrepareRewardsModel(command);
            model.Title = settings.Title;
            model.Description = settings.Description;
            model.Picture = _pictureService.GetPictureUrl(settings.ImageId);

            return View("RewardTemplate.ProductsInGridOrLines", model);
        }

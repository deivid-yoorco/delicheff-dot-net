    ...
    public partial class CatalogPagingFilteringModel : BasePageableModel
    {
        ...
        public partial class SpecificationFilterModel : BaseNopModel
        {
            ...
            public virtual void PrepareSpecsFilters(IList<int> alreadyFilteredSpecOptionIds,
                ...
                IWebHelper webHelper, IWorkContext workContext, ICacheManager cacheManager, IPagedList<Product> products)
            {
                Enabled = false;
                ...
                var allFilters = allOptions.Select(sao =>
                    new SpecificationAttributeOptionFilter
                    {
                        ...
                        SpecificationAttributeIcon = sao.Icon
                    }).ToList();
                
                if (!allFilters.Any())
                    return;

                var specificationAttributes = products.Select(x => x.ProductSpecificationAttributes);
                foreach (var item in allFilters)
                {
                    var count = 0;
                    foreach (var specificationAttribute in specificationAttributes)
                    {
                        count = count + specificationAttribute.Where(z => z.SpecificationAttributeOptionId == item.SpecificationAttributeOptionId).Count();
                    }
                    item.SpecificationAttributeOptionName = item.SpecificationAttributeOptionName + " (" + count + ")";
                }

                //sort loaded options
                allFilters = allFilters.OrderBy(saof => saof.SpecificationAttributeDisplayOrder)
                    .ThenBy(saof => saof.SpecificationAttributeName)
                    .ThenBy(saof => saof.SpecificationAttributeIcon)
                ...
                AlreadyFilteredItems = alreadyFilteredOptions.Select(x =>
                    new SpecificationFilterItem
                    {
                        ...
                        SpecificationAttributeIcon = x.SpecificationAttributeIcon
                    }).ToList();
        ...
        public partial class SpecificationFilterItem : BaseNopModel
        {
            ...
            public string SpecificationAttributeIcon { get; set; }
        }

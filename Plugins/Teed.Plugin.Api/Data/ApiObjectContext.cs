...
using Teed.Plugin.Api.Data.Mapping;
using Teed.Plugin.Api.Domain.Identity;

namespace Teed.Plugin.Api.Data
{
    public class ApiObjectContext : DbContext, IDbContext
    {
        public ApiObjectContext(string nameOrConnectionString) : base(nameOrConnectionString) { }

        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            modelBuilder.Configurations.Add(new CustomerSecurityTokenMap());
            if (Nop.Services.TeedCommerceStores.CurrentStore == Nop.Services.TeedStores.CentralEnLinea)
            {
                modelBuilder.Configurations.Add(new WebScrapingUnitMap());
                modelBuilder.Configurations.Add(new WebScrapingHistoryMap());
            }
            modelBuilder.Configurations.Add(new TaggableBoxMap());//new
            modelBuilder.Configurations.Add(new PopupMap());
            modelBuilder.Configurations.Add(new OnboardingMap());
            modelBuilder.Configurations.Add(new CustomerSavedCardMap());

...
namespace Nop.Plugin.ExternalAuth.Facebook.Controllers
{
    public class FacebookAuthenticationController : BasePluginController
    {
        ...
        public async Task<IActionResult> LoginCallback(string returnUrl)
        {
            ...
            else if (authenticateResult.Succeeded && authenticateResult.Principal.Claims.Any())
            {
                var customer = _customerService.GetCustomerByEmail(authenticateResult.Principal.FindFirst(claim => claim.Type == ClaimTypes.Email)?.Value);
                var facebookReward = _customerPointService.GetCustomerPointsByCustomerId(customer.Id)
                    .Where(x => x.RewardSystemName == "facebook-first-login").FirstOrDefault();
                if (facebookReward == null)
                {
                    RewardSettings settings = _settingService.LoadSetting<RewardSettings>();
                    if (settings.FacebookPoints > 0 && settings.IsActive)
                        _customerPointService.InsertCustomerPoint(new CustomerPoint
                        {
                            CustomerId = customer.Id,
                            Description = "¡Iniciaste sesión por primera vez con Facebook!",
                            Points = settings.FacebookPoints,
                            RewardSystemName = "facebook-first-login",
                            IsChallenge = false
                        });
                }
            }
        ...
        [HttpGet]
        public IActionResult GetFacebookPoints()
        {
            RewardSettings settings = _settingService.LoadSetting<RewardSettings>();
            if (settings.FacebookPoints > 0 && settings.IsActive)
                return Json(settings.FacebookPoints);
            else
                return Json(0);
        }
